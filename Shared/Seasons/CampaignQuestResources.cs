...
public static class CampaignQuestResources
{
    ...
    {
        var quests = season.Quests.Where(q => q.SeasonalEnabled != false).ToDictionary(q => q.Id);
        foreach (var loot in season.QuestLoot)
        {
            if (
                ...
                || loot.BotRoles.Any(string.IsNullOrWhiteSpace)
            )
        ...
        foreach (var craft in season.Crafts)
        {
            ...
            var gates = craft.Requirements.Where(r => r.Type == "QuestComplete").ToArray();
            if (
                gates.Length != 1
                || !quests.TryGetValue(gates[0].QuestId ?? "", out var quest)
                || !quest.AllRewards().Any(r => r.Type == "ProductionScheme" && r.Items.Any(i => i.Template == craft.EndProduct))
            )
        ...
        foreach (var quest in quests.Values.Where(_ => !season.Legacy))
        foreach (var reward in quest.AllRewards().Where(r => r.Type == "ProductionScheme"))
            if (
                !season.Crafts.Any(c => c.Requirements.Any(r => r.QuestId == quest.Id) && reward.Items.Any(i => i.Template == c.EndProduct))
            )
