...
public static class TraderPricing
{
    ...
    public static double Scale(double count, decimal multiplier) => (double)((decimal)count * multiplier);

    // EFT.Trading.Requisite rounds after multiplying by the requested quantity.
    public static double Required(double unitCount, int quantity) => Math.Ceiling(unitCount * quantity);
}
