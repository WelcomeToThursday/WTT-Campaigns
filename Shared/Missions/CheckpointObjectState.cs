...
public sealed class CheckpointObjectState
{
    ...
    public CheckpointObjectState(IEnumerable<object> roots, Func<object, bool> owns, Func<object, FieldInfo, bool>? member = null)
    {
        _owns = owns;
        _member = member;
        foreach (var root in roots) Capture(root, true);
    }
    ...
    public void Restore(IEnumerable<KeyValuePair<object, object>>? replacements = null)
    {
        ...
        object? Resolve(object? value, object? current = null)
        {
            if (value == null)
            {
                ...
                if (current is Delegate existing)
                    foreach (var invocation in existing.GetInvocationList())
                        if (invocation.Target == null || !_owns(invocation.Target)) external = Delegate.Combine(external, invocation);
                return external;
            }
            if (map.TryGetValue(value, out var replacement)) return replacement;
            ...
            if (value is Delegate callback)
            {
                ...
                foreach (var invocation in callback.GetInvocationList())
                {
                    ...
                    if (target != null && !map.ContainsKey(target) && !_entries.ContainsKey(target) && !live.Contains(invocation)) continue;
                    var rebound = target != null && map.TryGetValue(target, out var newTarget)
                        ? Delegate.CreateDelegate(invocation.GetType(), newTarget, invocation.Method) : invocation;
                    result = Delegate.Combine(result, rebound);
                }
                foreach (var invocation in live)
                    if ((invocation.Target == null || !_owns(invocation.Target))
                        && !(result?.GetInvocationList().Contains(invocation) ?? false)) result = Delegate.Combine(result, invocation);
                return result;
            }
            return value;
        }
        ...
        {
            if (!entry.JsonToken) continue;
            var target = (JToken)Resolve(entry.Target)!;
            if (target is JProperty property) property.Value = JValue.CreateNull();
            else if (target is JContainer container) container.RemoveAll();
            else if (target is JValue scalar) scalar.Value = Resolve(entry.JsonValue);
        }
        foreach (var entry in _entries.Values)
        {
            if (!entry.JsonToken || entry.Items == null) continue;
            var target = (JContainer)Resolve(entry.Target)!;
            foreach (var saved in entry.Items)
            {
                var child = (JToken)Resolve(saved)!;
                if (child.Parent is JProperty parent) parent.Value = JValue.CreateNull();
                else if (child.Parent != null) child.Remove();
                if (target is JProperty property) property.Value = child;
        ...
        foreach (var entry in _entries.Values)
        {
            ...
            if (entry.Pairs != null)
            {
                ...
                dictionary.Clear();
                foreach (var pair in entry.Pairs) dictionary.Add(Resolve(pair.Key)!, Resolve(pair.Value));
            }
            else if (entry.Items != null)
            {
                if (target is Array array)
                {
                    for (var index = 0; index < entry.Items.Length; index++) array.SetValue(Resolve(entry.Items[index]), index);
                }
                else if (target is IList list)
                {
                    if (list.IsFixedSize)
                    {
                        if (list.Count != entry.Items.Length) throw new InvalidOperationException("Native fixed checkpoint collection changed size.");
                        for (var index = 0; index < entry.Items.Length; index++) list[index] = Resolve(entry.Items[index]);
                    }
                    else
                    {
                        list.Clear();
                        foreach (var item in entry.Items) list.Add(Resolve(item));
                    }
                }
                else
                {
                    entry.SetClear!.Invoke(target, null);
                    foreach (var item in entry.Items) entry.SetAdd!.Invoke(target, new[] { Resolve(item) });
                }
            }
        }
    }
    ...
    {
        if (value == null || value is string or Type or MemberInfo) return;
        var type = value.GetType();
        if (type.IsValueType)
        {
            if (!Scalar(type))
                foreach (var field in type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)) Capture(field.GetValue(value));
            return;
        }
        if (value is Delegate callback)
        {
            foreach (var invocation in callback.GetInvocationList())
                if (invocation.Target?.GetType().IsDefined(typeof(CompilerGeneratedAttribute), false) == true) Capture(invocation.Target);
            return;
        }
        if (_entries.ContainsKey(value) || (!root && !_owns(value))) return;
        if (_entries.Count >= 200000) throw new InvalidOperationException("Native checkpoint state exceeded its capture limit.");
        ...
        if (value is JToken token)
        {
            ...
            if (token is JContainer container)
            {
                ...
                foreach (var child in entry.Items) Capture(child, true);
            }
            ...
            else throw new InvalidOperationException("Unsupported checkpoint JSON token: " + type.Name);
            return;
        }
        if (value is IDictionary dictionary && !dictionary.IsReadOnly && !dictionary.IsFixedSize)
        {
            ...
            foreach (DictionaryEntry pair in dictionary)
            {
                ...
                Capture(pair.Key); Capture(pair.Value);
            }
            return;
        }
        if (value is IList list && (!list.IsReadOnly || value is Array))
        {
            if (value is Array array && array.Rank != 1) throw new InvalidOperationException("Unsupported native checkpoint array rank.");
            entry.Items = new object?[list.Count];
            list.CopyTo(entry.Items, 0);
            foreach (var item in entry.Items) Capture(item);
            return;
        }
        ...
        for (var current = type; current != null && current != typeof(object); current = current.BaseType)
            ...
            {
                if (_member != null && !_member(value, field)) continue;
