...
public static class MissionAcknowledgement
{
    ...
    {
        if (returned == null)
            throw new InvalidOperationException("Missing checkpoint acknowledgement.");
        var generation = preparing ? checked(expected.AttemptGeneration + 1) : expected.AttemptGeneration;
        if (
            expected.Status != MissionRunStatuses.Active
            || returned.Status != MissionRunStatuses.Active
            || returned.AttemptGeneration != generation
            || returned.Restoring != preparing
            || expected.Restoring == preparing
        )
            throw new InvalidOperationException(
                $"Checkpoint {(preparing ? "prepare" : "commit")} was not acknowledged: "
    ...
    public static void Require(MissionRun expected, MissionRun? returned, bool committed)
    {
        if (
            ...
            || returned.LayoutId != expected.LayoutId
            || returned.ContentRevision != expected.ContentRevision
            || returned.ContentHash != expected.ContentHash
            || returned.AttemptGeneration != expected.AttemptGeneration
            || returned.Restoring != expected.Restoring
            || returned.Status != expected.Status
        )
