...
public static class MissionLibrary
{
    ...
    public static string LockReason(CampaignMissionLink link) =>
        link.Availability switch
        {
    ...
    public static void ValidatePackage(SeasonDefinition package, SeasonValidationResult result)
    {
        ...
        Need(
            ...
            "Enter a mission package name (up to 120 characters)."
        );
        Need(
            package.MissionLinks.Count == 0 && package.Story == null && package.Quests.Count == 0,
            "Independent missions cannot contain campaign links or story quests."
        );
        Need(
            package.Documents.Count == 0
                ...
                && package.Crates.Count == 0
                && package.Crafts.Count == 0
                && package.QuestLoot.Count == 0
                && !package.Legacy,
            "Mission packages contain mission content, not campaign rewards, traders or progression."
        );
        Need(
            package.Dependencies.Count <= 1000 && package.Items.Count <= 1000 && package.ImportedItems.Count <= 1000,
        ...
        if (package.Missions.Count != 1 || package.MapLayouts.Count != 1)
            return;
    ...
    {
        if (campaign.MissionLinks.Count > 1000)
        {
        ...
        foreach (var link in campaign.MissionLinks)
        {
            ...
            if (link.Package.MissionPackage == null || link.Package.MissionLinks.Count != 0)
                continue;
            ValidatePackage(link.Package, result);
            Need(
                link.Revision > 0
                    && link.Revision == link.Package.Revision
                    && link.Package.Missions.Count == 1
                    && link.Package.Missions[0].Id == link.MissionId,
            ...
            if (link.QuestId.Length > 0)
            {
                ...
                Need(
                    condition?.ConditionType == "GlobalVariableValue"
                        && condition.Value is >= 1 and <= 1000000
                        && (condition.CompareMethod == null || condition.CompareMethod == ">=")
                        && variable?.Scope == StoryVariableScope.Profile
                        && variable.InitialValue == 0,
                    "Choose a completion objective backed by a zero-initialized profile variable."
                );
            }
            else
                Need(link.CompletionConditionId.Length == 0, "Choose a quest before its completion objective.");
        }
    }
    ...
    public static string PackageHash(SeasonDefinition package)
    {
        ...
        return BitConverter
            .ToString(hash.ComputeHash(System.Text.Encoding.UTF8.GetBytes(SeasonCompiler.GameplayIdentity(package))))
            .Replace("-", "")
            .ToLowerInvariant();
    }
        ...
        foreach (var link in source.MissionLinks)
        {
            ...
            Serialization.ModelGraph.Rewrite(
                ...
                text =>
                {
                    ...
                    return colon == 24 && replacements.TryGetValue(text.Substring(0, colon), out value)
                        ? value + text.Substring(colon)
                        : text;
                }
            );
