...
def main():
    ...
    check(all(p['imageUrl'].startswith('/wtt-campaigns/icons/') for p in all_perks),'Every artwork URL is local')
    ...
    created=request('/wtt-campaigns/create',request_data,root)
    ...
    bad=request('/wtt-campaigns/edit',{'PerkIds':[names['Hercules']],'ExpectedRevision':1},root)
    check(bool(bad.get('Error')),'Reject insufficient budget')
    bad=request('/wtt-campaigns/edit',{'PerkIds':[names['Lucky']],'ExpectedRevision':1},root)
    check(bool(bad.get('Error')),'Reject unsupported perk')
    bad=request('/wtt-campaigns/edit',{'PerkIds':[names['Hemophilia'],names['Thrombophilia']],'ExpectedRevision':1},root)
    check(bool(bad.get('Error')),'Reject mutually exclusive perks on the server')
    bad=request('/wtt-campaigns/edit',{'PerkIds':selected,'ExpectedRevision':0},root)
    check(bool(bad.get('Error')),'Reject stale edit')
    edited=request('/wtt-campaigns/edit',{'PerkIds':selected,'ExpectedRevision':1},root)
    ...
    for mode in ['normal','seasonal','normal']:
        result=request('/wtt-campaigns/switch',{'Mode':mode},root)
