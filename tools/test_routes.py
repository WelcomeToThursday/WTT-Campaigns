...
def main():
    ...
    def seasonal(path, payload=None, session=root):
        return request('/wtt-seasonal/' + path, {**identity, **(payload or {})}, session)

    for perk in snapshot['Catalogue']['common'] + snapshot['Catalogue']['personal']:
        check(perk['imageUrl'] == '/wtt-seasonal/icons/' + perk['id'] + '.png', 'Catalogue publishes the new icon route')
    ...
    check(request('/wtt-seasonal/hub-images/' + asset + '.png', raw=True).startswith(b'\x89PNG\r\n\x1a\n'),
    ...
    page = request('/wtt-seasonal/creator', raw=True).decode('utf-8')
    check('Season Creator' in page and '/wtt-seasonal/creator/packs/' in page, 'Creator page renders new export links')
    check('/seasonal-perks/' not in page, 'Creator page contains no old local routes')
    check(request('/wtt-seasonal/creator/assets/' + asset + '.png', raw=True).startswith(b'\x89PNG\r\n\x1a\n'),
          'Creator asset route serves PNG bytes')
    selection = json.loads((SERVER / 'user/mods/SeasonalPerks/creator/selection.json').read_text(encoding='utf-8-sig'))
    if selection['Active']:
        pack = request('/wtt-seasonal/creator/packs/' + selection['Active'] + '/download', raw=True)
