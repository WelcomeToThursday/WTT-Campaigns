...
def main():
    ...
    after_rejected = request('/seasonal-perks/snapshot', session=root)
    ...
    check(request('/seasonal-perks/snapshot', session=root)['ActiveMode'] == 'normal', 'Switching back to normal succeeds')
    request('/seasonal-perks/switch', {'Mode': 'seasonal'}, root)
    check(request('/seasonal-perks/snapshot', session=root)['State']['SeasonalPerkEffectParameters'] == parameters, 'Switching preserves deterministic target data')
