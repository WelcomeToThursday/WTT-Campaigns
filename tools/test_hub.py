...
def main():
    ...
    request('/wtt-campaigns/switch', {'Mode': 'seasonal'}, account['root'])
    ...
    data = request('/wtt-campaigns/hub', session=account['child'])
    ...
    for image in json.loads((PROJECT / 'data/hub-images.json').read_text()):
        raw = request('/wtt-campaigns/hub-images/' + image['Id'] + '.png', raw=True)
