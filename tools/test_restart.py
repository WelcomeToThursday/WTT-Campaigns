...
def main():
    ...
    child_snapshot = request('/seasonal-perks/snapshot', session=child)
    check(child_snapshot==snapshot, 'Launching seasonal profile resolves its original account')
    check(bool(request('/seasonal-perks/switch', {'Mode':'normal'}, root).get('Error')), 'Raid marker blocks account switching after restart')
    mutation = {'PerkIds':state['selected'], 'ExpectedRevision':state['revision']}
    check(bool(request('/seasonal-perks/edit', mutation, child).get('Error')), 'Raid marker blocks perk editing through seasonal identity')
    request('/client/game/start', session=child)
    edited = request('/seasonal-perks/edit', mutation, child)
    ...
    normal = request('/seasonal-perks/switch', {'Mode':'normal'}, child)
