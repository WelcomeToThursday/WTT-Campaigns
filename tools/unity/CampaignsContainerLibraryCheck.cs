...
public static class CampaignsContainerLibraryCheck
{
 public static void Run()
 {
  ...
  if(!bundle) throw new Exception("Container bundle failed to load");
  int count=0;
  foreach(var name in bundle.GetAllAssetNames())
  {
   Debug.Log("CHECKING CONTAINER " + name);
   var prefab=bundle.LoadAsset<GameObject>(name);
   if(!prefab)throw new Exception("Missing prefab "+name);
   ...
   if(!prefab.GetComponentsInChildren<MeshRenderer>(true).Any())throw new Exception("No model "+name);
   if(prefab.GetComponentsInChildren<MeshFilter>(true).Any(m=>!m.sharedMesh))throw new Exception("Missing mesh "+name);
   if(prefab.GetComponentsInChildren<Renderer>(true).Any(r=>r.enabled && Active(r.transform) && r.sharedMaterials.Any(m=>!m||!m.shader)))throw new Exception("Missing material "+name);
   Debug.Log("CONTAINER VERIFIED "+name+" "+prefab.name);count++;
  }
  bundle.Unload(true);
