...
{
    public static class CampaignsStoryNotificationBuilder
    {
        ...
        public static void Build()
        {
            ...
            var existing = AssetBundle.LoadFromFile(Path.Combine(output, "wtt_campaigns_ui.bundle"));
            if (!existing)
            ...
            File.Copy(Path.Combine(output, "wtt_campaigns_ui.bundle"), Path.Combine(backup, "wtt_campaigns_ui.bundle"));
            var manifest = BuildPipeline.BuildAssetBundles(
                ...
                new[]
                {
                    ...
                    {
                        assetBundleName = "wtt_campaigns_ui.bundle",
                        assetNames = mainAssets.Select(p => p.Replace('\\', '/')).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(),
                    },
                    new AssetBundleBuild { assetBundleName = "wtt_campaigns_story_notifications.bundle", assetNames = prefabs.ToArray() },
                },
                BuildAssetBundleOptions.ChunkBasedCompression,
                BuildTarget.StandaloneWindows64
            );
            if (!manifest || !manifest.GetAllDependencies("wtt_campaigns_story_notifications.bundle").Contains("wtt_campaigns_ui.bundle"))
                throw new InvalidDataException("Chapter prefab bundle must depend on the main UI's shared status icons.");
            var dependencies = manifest.GetAllDependencies("wtt_campaigns_story_notifications.bundle");
            var main = AssetBundle.LoadFromFile(Path.Combine(output, "wtt_campaigns_ui.bundle"));
            var notifications = AssetBundle.LoadFromFile(Path.Combine(output, "wtt_campaigns_story_notifications.bundle"));
            try
            {
            ...
            var checks = new JObject
            {
                ...
                ["bundles"] = new JArray(
                    new[] { "wtt_campaigns_ui.bundle", "wtt_campaigns_story_notifications.bundle" }.Select(name => new JObject
                    {
                        ["file"] = name,
