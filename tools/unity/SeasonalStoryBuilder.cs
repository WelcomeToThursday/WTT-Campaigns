...
public static class SeasonalStoryBuilder
{
    private const string Root = "Assets/Mods/SeasonalPerks.Assets/StoryTraders";
    ...
    {
        BuildRoom(SeasonalPeacekeeperBuilder.TraderId);
    }
    ...
    private static void BuildRoom(string selected)
    {
        ...
        if (selected == null || selected == SeasonalPeacekeeperBuilder.TraderId)
        {
            var peacekeeper = SeasonalPeacekeeperBuilder.Create();
            var peacekeeperPath = Root + "/" + SeasonalPeacekeeperBuilder.TraderId + ".prefab";
            PrefabUtility.SaveAsPrefabAsset(peacekeeper, peacekeeperPath);
            audit.Add(
                new JObject
                {
                    ["trader"] = SeasonalPeacekeeperBuilder.TraderId,
            ...
            builds.Add(
                ...
                {
                    assetBundleName = "traders/" + SeasonalPeacekeeperBuilder.TraderId + ".bundle",
