...
def main():
    ...
    snapshot = request("/wtt-campaigns/snapshot", base, root)
    check(snapshot.get("HasStory"), "Active test pack advertises story support")
    created = request("/wtt-campaigns/create", {**base, "OperationId": secrets.token_hex(16), "Nickname": "StoryTest", "Side": "Usec", "PerkIds": [fixture["PerkId"]]}, root)
    ...
    check(request("/wtt-campaigns/story", {**identity, "CharacterId": root}, child).get("Error"), "Wrong-character story read rejected")
    check(request("/wtt-campaigns/story", {**identity, "SeasonId": "0" * 24}, child).get("Error"), "Wrong-season story read rejected")
    ...
    replay = request("/wtt-campaigns/story/select", accepted, child)
    ...
    check(request("/wtt-campaigns/story/select", changed, child).get("Error"), "Operation identifier cannot be reused for another choice")
    ...
    request("/wtt-campaigns/story/select", completed, child)
    ...
    old = request("/wtt-campaigns/story/select", accepted, child)
    ...
    checkpoint = read()
    normal = request("/wtt-campaigns/switch", {**base, "Mode": "normal"}, root)
    check(not normal.get("Error"), "Normal character can be selected after story progress")
    check(request("/wtt-campaigns/story", identity, root).get("Error"), "Normal sessions cannot read a Seasonal story")
    check(request("/wtt-campaigns/story/select", accepted, child).get("Error"), "Inactive Seasonal sessions cannot replay story mutations")
    restored = request("/wtt-campaigns/switch", {**base, "Mode": "seasonal", "CharacterId": character}, root)
    ...
    legacy_pack = SERVER / "user/mods/WTT-Campaigns/creator/legacy.json"
    legacy_bytes = legacy_pack.read_bytes()
    legacy_switch = request("/wtt-campaigns/switch", {"ProtocolVersion": 2, "SeasonId": legacy_season, "Mode": "seasonal", "CharacterId": legacy_child}, root)
    ...
    empty_story = request("/wtt-campaigns/story", legacy_identity, legacy_child)
    ...
    check(request("/wtt-campaigns/story/start", empty_start, legacy_child).get("Error"), "Empty story cannot start an unauthored conversation")
