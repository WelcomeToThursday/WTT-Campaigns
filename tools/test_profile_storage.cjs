...
async function main(){
 ...
 if(mode==='create'){
  ...
  const initial=await request('/wtt-campaigns/snapshot',{ProtocolVersion:2},root);
  ...
  const created=await request('/wtt-campaigns/create',payload,root);
  ...
  const replay=await request('/wtt-campaigns/create',payload,root);
 ...
 const call=async(action,data={})=>{const r=await request('/wtt-campaigns/'+action,{ProtocolVersion:2,...data},root);check(!r.Error,action+': '+(r.Error||'succeeded'));return r;};
 ...
 const hub=await request('/wtt-campaigns/hub',{ProtocolVersion:2},child);
