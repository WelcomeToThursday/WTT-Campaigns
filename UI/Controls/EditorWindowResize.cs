...
public sealed class EditorWindowResize : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnDrag(PointerEventData e)
    {
        ...
        var size = new Vector2(
            Mathf.Clamp(
                _size.x + delta.x,
                Mathf.Min(Minimum.x, Boundary.rect.width - 16),
                Boundary.rect.xMax - (_position.x - _size.x / 2) - 8
            ),
            Mathf.Clamp(
                _size.y - delta.y,
                Mathf.Min(Minimum.y, Boundary.rect.height - 92 - BottomInset),
                _position.y + _size.y / 2 - Boundary.rect.yMin - BottomInset
            )
        );
