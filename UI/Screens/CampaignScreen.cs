...
public sealed partial class CampaignScreen : IDisposable
{
    ...
    public CampaignScreen(Transform parent, Func<string, GameObject> prefab, Font font, bool embedded = false)
    {
        _ui = new UiElements(font, sound => SoundRequested?.Invoke(sound));
        _prefab = prefab;
        _embedded = embedded;
        var root = UiElements.Rect("CampaignsScreen", parent, 0, 0);
