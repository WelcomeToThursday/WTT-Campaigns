...
public static class RaidEditorLayout
{
    ...
    {
        var root = new GameObject("SeasonalRaidEditor", typeof(RectTransform), typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster));
        ...
        Button(ui, left, "Zones", "Zones", 72, -117, 344); Button(ui, left, "Bindings", "Events", 72, -39, 344);
        Button(ui, left, "Captures", "Captures", 72, 39, 344); Button(ui, left, "Scene", "Scene", 72, 117, 344);
        ...
        Button(ui, left, "Previous", "Previous", 145, -80, -211); Button(ui, left, "Next", "Next", 145, 80, -211);
        Button(ui, left, "AddBox", "+ Box", 145, -80, -265); Button(ui, left, "AddSphere", "+ Sphere", 145, 80, -265);
        Button(ui, left, "Capture", "Capture transform", 306, 0, -313);
        Button(ui, left, "Pick", "Pick scene object", 306, 0, -361);
        Button(ui, left, "Undo", "Undo", 145, -80, -417); Button(ui, left, "Redo", "Redo", 145, 80, -417);
        ...
        Button(ui, right, "Move", "Move", 87, -148, -29); Button(ui, right, "Rotate", "Rotate", 87, -50, -29);
        Button(ui, right, "Scale", "Resize", 87, 50, -29); Button(ui, right, "Snap", "Snap: on", 87, 148, -29);
        Button(ui, right, "AtFeet", "At player", 185, -100, -80); Button(ui, right, "AtAim", "At aim point", 185, 100, -80);
        Button(ui, right, "InZone", "In zone", 122, -133, -130); Button(ui, right, "VisitPlace", "Visit", 122, 0, -130); Button(ui, right, "LeaveItemAtLocation", "Place item", 122, 133, -130);
        ui.Label(right, "Details", "Draft previews never execute quests or story actions.", 15, 388, 110, 0, -211);
        Button(ui, right, "Parent", "Select parent", 185, -100, -293); Button(ui, right, "UseObject", "Use scene target", 185, 100, -293);
        Button(ui, right, "Duplicate", "Duplicate", 185, -100, -343); Button(ui, right, "Delete", "Delete", 185, 100, -343);
        Button(ui, right, "Complete", "Complete capture", 250, -68, -409); Button(ui, right, "Cancel", "Cancel", 124, 132, -409);
        ...
        ui.Label(bottom, "Help", "WASD / Q E · Shift boost · RMB look\nDrag axis handles · Ctrl+Z / Ctrl+Y · Escape cancels, then closes", 17, 680, 70);
    ...
    {
        var rect = UiElements.Rect(name, parent, width, height); rect.anchorMin = rect.anchorMax = anchor; rect.anchoredPosition = position;
        UiElements.Fill(rect, new Color(.045f, .047f, .042f, .96f), true); return rect;
    }
    ...
    {
        var button = ui.Button(parent, text, width, x, y, () => { }, height); button.name = name;
        var label = button.GetComponentInChildren<Text>(); label.resizeTextForBestFit = true; label.resizeTextMinSize = 12; label.resizeTextMaxSize = 18;
    }
    ...
    private static void Multiline(UiElements ui, Transform parent, string name, float y)
    {
        ...
        field.textComponent.fontSize = 14; field.textComponent.alignment = TextAnchor.UpperLeft;
        field.lineType = InputField.LineType.MultiLineNewline; field.readOnly = true; field.characterLimit = 0;
    }
}
