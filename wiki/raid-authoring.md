...
The Scene catalog has an **All game / Current map** source dropdown and **Props / Containers / Loot / Presets** categories. All game combines scenery indexed from the installed `level*` files and their `sharedassets` dependencies, discovered objects in the current map, and native inventory/container catalogs. The editor no longer scans registered bundles in the background. Search matches names, alternate source names, level names, bundle paths and asset paths. Current map limits scenery to the loaded map and inventory entries to discovered loot templates.

**Hide unavailable** is enabled initially. Turn it off to inspect unsupported entries and their placement restrictions. Containers awaiting native loot mappings remain hidden until validated; the catalog reports their pending count. A missing thumbnail does not make a placeable asset unavailable. Loot and preset availability is filtered before paging.

The level scenery library is generated locally during the client build from read-only game files. It includes inactive scenery, preserves source orientation, LOD groups and supported colliders, and deduplicates repeated geometry. Meshes and textures are shared across generated props. Gameplay-owned objects, unsupported components, combined static geometry and invalid dependencies remain unavailable with reasons. Catalog filtering and sorting run off the Unity thread; selecting a prop loads its required generated resources without opening its source scene.

The catalog uses the exported level library and does not inspect registered game/mod bundles at runtime. There are no bundle-scan, pause or rescan controls. Current-map catalog discovery still examines loaded objects, including beyond the separate scene-binding limit. Rebuild/install the client after updating game level files to regenerate the local scenery library. Existing saved bundle references remain loadable on demand, with dependency checks and placement restrictions intact.

Picking a prop in the world preserves the current browser tab and updates the inspector and transform controls independently. Exported level props draw their selection outline in the original model's axes, including its source rotation and scale, without changing the saved placement transform.
