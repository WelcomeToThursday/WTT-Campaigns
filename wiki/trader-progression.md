...
Existing reputation and task progress are preserved. Loyalty is recalculated against the new requirements and can decrease. Accepted tasks remain active after a loyalty loss, while unaccepted tasks can become locked. Completed tasks do not pay reputation again.

One-time tasks alone may not provide enough reputation for every trader's maximum loyalty. Repeatable tasks and other reputation sources can still matter.

## Tasks

Supported existing tasks use updated reputation rewards, penalties, loyalty requirements and trader assignments. This does not add missing quests or replace their objectives, item rewards or XP rewards. Custom tasks retain their own progression.
