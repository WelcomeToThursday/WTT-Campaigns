...
[Injectable]
public class MedicalResourcePatch(
    ItemHelper items,
...
) : AbstractPatch
{
    ...
    )
    {
        ...
        var body =
            request.Part == null
                ? null
                : pmcData.Health?.BodyParts?.GetValueOrDefault(request.Part);
        if (
            properties == null
            || body?.Health == null
        ...
        {
            if (
                Enum.TryParse<DamageEffectType>(name, out var kind)
                && properties.EffectsDamage?.TryGetValue(kind, out var effect) == true
            )
            {
        ...
        body.Health.Current = Math.Min(
            body.Health.Maximum ?? 0,
            (body.Health.Current ?? 0) + remainingUnits
        );
        item.Upd ??= new Upd();
