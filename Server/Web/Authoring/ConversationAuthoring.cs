...
public static class ConversationAuthoring
{
    ...
    {
        return line.Trigger.Type == "VariableValue"
            && line.Trigger.Target == dialog.MainVariable
            && line.Trigger.Operator == "=="
            && line.Trigger.Value == Math.Truncate(line.Trigger.Value)
            && line.Trigger.Value >= int.MinValue
            && line.Trigger.Value < int.MaxValue;
    }

    public static IEnumerable<StoryDialogLine> NextLines(StoryDialog dialog, StoryDialogLine line)
    {
        return dialog.Lines.Where(next =>
            IsPhaseLine(dialog, next)
            && line.Actions.Any(a =>
                a.Type == StoryActionType.SetVariable && a.Target == dialog.MainVariable && a.Value == next.Trigger.Value
            )
        );
    }
    ...
    public static StoryDialogLine AddAfter(StoryDefinition story, StoryDialog dialog, StoryDialogLine source, string side)
    {
        ...
        if (source.Actions.Any(a => a.Type == StoryActionType.QuitAction))
        {
            throw new InvalidOperationException(
                "This line ends the conversation. Remove End conversation under Effects before adding a continuation."
            );
        }
        ...
        {
            throw new InvalidOperationException(
                "This reply already leads to a trader line. Select that line in the outline to edit it, or connect to a different line."
            );
        }
        ...
        var line = new StoryDialogLine
        {
            ...
            Side = side,
            Text = side == "Player" ? "New player reply" : "New trader line",
            Trigger = new()
            {
                Type = "VariableValue",
                Target = dialog.MainVariable,
                Operator = "==",
                Value = phase,
            },
        };
    ...
    private static void RequirePhase(StoryDefinition story, StoryDialog dialog, StoryDialogLine line)
    {
        ...
        {
            throw new InvalidOperationException(
                "Guided connections need this conversation's own Dialogue-scope phase variable. Use Conditions and Effects for imported or shared-variable logic."
            );
        }
        ...
        {
            throw new InvalidOperationException(
                "This line has a custom availability condition. Use Conditions and Effects to preserve its behavior."
            );
        }

        if (
            line.Actions.Count(a => a.Type == StoryActionType.SetVariable && a.Target == dialog.MainVariable) > 1
            || line.Actions.Any(a => a.Type is StoryActionType.SwitchDialog or StoryActionType.EmbedQuestDialog)
        )
        {
            throw new InvalidOperationException(
                "This line has a complex transition. Edit its actions under Effects to preserve their order and behavior."
            );
        }
    }
    ...
    private static int NextPhase(StoryDefinition story, StoryDialog dialog)
    {
        var used = ModelGraph
            .Texts(dialog)
            .SelectMany(t => t.Ancestors)
            .OfType<StoryCondition>()
            .Distinct()
            ...
            .Concat(
                dialog
                    ...
                    .Where(a => a.Type == StoryActionType.SetVariable && a.Target == dialog.MainVariable)
                    .Select(a => a.Value)
            )
            .Concat(dialog.StartPoints.Values)
            .Append(story.Variables.Single(v => v.Id == dialog.MainVariable).InitialValue)
            .ToHashSet();
        var phase = 0;
