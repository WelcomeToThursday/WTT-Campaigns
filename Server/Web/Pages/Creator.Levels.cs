...
{
    [Parameter] public bool LevelEditor { get; set; }
    [SupplyParameterFromQuery(Name = "layout")] public string? LayoutQuery { get; set; }
    private string _levelLayout = "";

    private void OpenLevel(EditorLayoutChoice choice) => Run(() =>
    {
        var draft = Repository.Load(choice.DraftId);
        if (!draft.Definition.MapLayouts.Any(l => l.Id == choice.Id && EditorContentRules.Mode(draft.Definition, l.Id) == EditorContentMode.Level))
    ...
    private void CheckLevelEdit()
    {
        ...
        EditorContentRules.ValidateEdit(Newtonsoft.Json.JsonConvert.DeserializeObject<WTT.Campaigns.Shared.Seasons.SeasonDefinition>(_baseline)!, S);
    }
}
