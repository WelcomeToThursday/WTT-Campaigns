...
{
    [Inject] private RaidAuthoringService RaidAuthoring { get; set; } = null!;
    ...
    private RaidEditorContext RaidContext
    {
        get
        {
            return new() { Connected = _draft != null && RaidAuthoring.Connected(_draft.Id), Request = RequestCapture };
        }
    }

    private void InputPending() { _inputPending = true; Dirty(); }
    private void InputCommitted() { _inputPending = false; Dirty(); }
    private Task SyncRaid()
    {
        if (_draft != null && !_inputPending && _raidConflict == null && RaidAuthoring.Connected(_draft.Id))
        {
            try { SyncDraft(); } catch (Exception e) { _raidStatus = e.Message; }
        }
    ...
    private void SyncDraft()
    {
        ...
        if (result.Conflicts.Count > 0) { _raidConflict = result; _raidStatus = "Choose which conflicting values to keep."; return; }
        if (result.Revision != _draft.Revision || DirtyState)
        {
            _draft = new DraftEnvelope { Id = result.DraftId, Revision = result.Revision, Definition = result.Definition! };
    ...
    {
        try { SyncDraft(); if (_raidConflict == null && _draft != null)
            {
                RaidAuthoring.Request(_draft.Id, task);
            }
        }
        catch (Exception e) { _raidStatus = e.Message; }
        return Task.CompletedTask;
    }
