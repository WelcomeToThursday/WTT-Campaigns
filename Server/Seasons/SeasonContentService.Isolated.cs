...
public sealed partial class SeasonContentService
{
    ...
    public IDisposable RegisterIsolated(SeasonDefinition definition)
    {
        ...
        var ids = definition
            .Items.Select(i => new MongoId(i.Id))
            .Concat(definition.ImportedItems.Keys.Select(id => new MongoId(id)))
            .ToHashSet();
        ...
        try
        {
            foreach (var language in localeTable.Global.Keys)
                localeRegistrations.Add(
                    _isolatedLocales.Install(language, locales.GetLocaleDb(language), SeasonCompiler.Texts(definition, language))
                );
