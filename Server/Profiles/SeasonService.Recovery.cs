...
public sealed partial class SeasonService
{
    ...
    private bool AccountExists(string id) => MongoId.IsValidMongoId(id)
        && (saves.ProfileExists(new MongoId(id)) || File.Exists(Path.GetFullPath(Path.Combine("user/profiles", id + ".json"))));

    public List<RecoveryAccount> RecoveryAccounts() => saves.GetProfiles()
        .Where(p => !SeasonProfileStorage.Contains(p.Key.ToString()) && !_ephemeralLinks.ContainsKey(p.Key.ToString())
    ...
    public List<RecoverableCharacter> RecoverableCharacters()
    {
        ...
        foreach (var pair in saves.GetProfiles())
        {
            ...
            if (state.Revision <= 0 || !MongoId.IsValidMongoId(owner) || owner == id || AccountExists(owner!))
                continue;
            result.Add(new(id, pmc.Info.Nickname ?? id, state.SeasonId ?? Seasons.SeasonRepository.LegacyId, owner!, pending?.TargetAccount ?? ""));
        }
        return result.OrderBy(c => c.Name).ToList();
    }

    private static CharacterRecoveryOperation? ReadRecovery(string id) => File.Exists(RecoveryPath(id))
    ...
    {
        if (!MongoId.IsValidMongoId(characterId) || !MongoId.IsValidMongoId(previousAccount)
            || !MongoId.IsValidMongoId(targetAccount) || characterId == previousAccount
            || characterId == targetAccount || previousAccount == targetAccount)
        ...
        if (pmc?.Info == null) throw new InvalidOperationException("This character has no recoverable PMC save.");
        var state = State(pmc);
        var pending = ReadRecovery(characterId);
        CharacterRecovery.ValidateOwnership(characterId, previousAccount, targetAccount, state.RootAccountId, state.Revision, AccountExists(previousAccount), pending);
        ...
        if (destination.ActiveRaidProfiles.Count > 0 || (destinationProfile.InraidData?.Location is { Length: > 0 } location && location != "none"))
            throw new InvalidOperationException("Finish the destination account's raid before recovering a character.");
        var entry = pending?.Character ?? source.Characters.FirstOrDefault(c => c.ProfileId == characterId)
            ?? new SeasonCharacterLink { ProfileId = characterId, SeasonId = state.SeasonId ?? Seasons.SeasonRepository.LegacyId, Created = true, Name = pmc.Info.Nickname ?? "" };
        ...
        if (pending == null)
        {
            ...
            BackupRecoveryFile(Path.Combine(SeasonProfileStorage.DirectoryPath, characterId + ".json"), Path.Combine(backup, "character.json"), true);
            foreach (var account in new[] { previousAccount, targetAccount })
                BackupRecoveryFile(Path.GetFullPath(Path.Combine("user/profileData", account, LinkKey + ".json")), Path.Combine(backup, account + ".json"), false);
            pending = new() { CharacterId = characterId, PreviousAccount = previousAccount, TargetAccount = targetAccount, Character = entry };
        ...
        SetState(pmc, state);
        if (profile.InraidData != null) profile.InraidData.Location = "none";
    ...
    private static void BackupRecoveryFile(string source, string destination, bool required)
    {
        ...
        {
            if (required) throw new IOException("The character save is missing on disk; recovery was not started.");
            return;
        }
