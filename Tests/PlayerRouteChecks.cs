...
internal static class PlayerRouteChecks
{
    ...
    {
        var route = new MapLayout { Start = new() { Id = "start" }, Exit = new() { Id = "exit", Name = "Gate" } };
        ...
        check(route.Checkpoints.SequenceEqual(new[] { a, b, c }), "Inserting into a route preserves traversal order and existing checkpoint identities");
        ...
        check(PlayerRoute.Progress(route, 3) == "Head to exit · Gate", "Reaching the last checkpoint directs the player to the exit instead of checkpoint 4 of 3");
