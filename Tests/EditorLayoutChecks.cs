...
internal static class EditorLayoutChecks
{
    public static void Run(Action<bool, string> check)
    {
        ...
        check(!EditorFieldGuide.NativeHelp(condition, "value").Contains("consumed"), "Changing condition type removes handover-specific guidance");
        condition["onlyFoundInRaid"] = true;
        check(EditorFieldGuide.NativeHelp(condition, "onlyFoundInRaid").StartsWith("Only found-in-raid"), "Quality help follows the raid-origin toggle");
        condition["minDurability"] = 30;
        condition["maxDurability"] = 80;
        check(EditorFieldGuide.NativeHelp(condition, "minDurability").Contains("30% to 80%"), "Durability help includes both current bounds");
        ...
        check(EditorFieldGuide.NativeHelp(distance, "value").Contains("75 metres"), "Nested distance uses metres rather than objective count");
        ...
        check(EditorFieldGuide.NativeHelp(effect, "multiplicator").Contains("energy last longer"), "Energy effect help explains why a lower multiplier helps");
        effect["effectId"] = "stamina_restore_body_parts_multiplicator";
        check(EditorFieldGuide.NativeHelp(effect, "multiplicator").Contains("Higher values restore"), "Recovery effect help reverses the beneficial direction");

        var season = new SeasonDefinition { Story = new StoryDefinition() };
        season.Story.Variables.Add(new StoryVariable { Id = "phase", InitialValue = 7, Scope = StoryVariableScope.Profile });
        var test = new StoryCondition { Type = "VariableValue", Target = "phase", Value = 9, Operator = "==" };
        var help = EditorFieldGuide.StoryHelp(test, "Value", season)!;
        check(help.Contains("== 9") && help.Contains("Profile scope, initial value 7"), "Variable help shows current comparison and selected variable declaration");
        ...
        check(EditorFieldGuide.StoryHelp(test, "Target", season)!.Contains("not partial progress"), "Objective state explains boolean completion instead of numeric progress");
        test.Type = "TraderReputation";
        check(EditorFieldGuide.StoryLabel(test, "Value") == "Reputation threshold" && EditorFieldGuide.StoryHelp(test, "Value")!.Contains("decimal"), "Reputation condition uses decimal-specific units and label");
