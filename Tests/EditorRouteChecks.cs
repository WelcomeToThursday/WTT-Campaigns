...
internal static class EditorRouteChecks
{
    internal static void Native(AssemblyDefinition native, AssemblyDefinition client)
    {
        ...
        Require(Reads(capture, "_flyPosition") && Reads(capture, "_flyRotation") && !Reads(capture, "_player"),
            "Route placement must follow the free camera even when the player remains at raid spawn.");
        Require(Calls(capture, "TryRouteFloor") && !Calls(capture, "PlayerScene"),
            "Route markers must bind to the floor at the camera, including its loaded scene.");
        Require(Calls(editor.Methods.Single(m => m.Name == "CaptureVolume"), "CapturePoint"),
        ...
        Require(constructor.Any(i => i.OpCode.Code == Code.Stfld && i.Operand is FieldReference f
            && f.Name == "_walkFromStart" && i.Previous.OpCode.Code == Code.Ldc_I4_1),
        ...
        Require(CallIndex("Apply") < CallIndex("SyncTransforms") && CallIndex("SyncTransforms") < CallIndex("ClearPosition"),
            "Spawn clearance must see the applied layout's current colliders.");
        Require(CallIndex("Close") >= 0 && CallIndex("Teleport") > CallIndex("Close"),
            "Native camera restoration must finish before walkthrough teleport.");
        Require(Reads(editor.Methods.Single(m => m.Name == "BeginWalkthrough"), "_flyPosition")
        ...
        Require(Reads(editor.Methods.Single(m => m.Name == "RestoreWalkCamera"), "_walkCameraRaid"),
        ...
        Require(EndsFrameAfterWalkthroughExit(update),
            "Walkthrough exit must end this Update before reopening or handling Escape again.");
        ...
        Require(end.Next.OpCode.Code == Code.Ldc_I4_1 && end.Next.Next.OpCode.Code == Code.Ret,
        ...
        var poll = editor.NestedTypes.Single(t => t.Name.StartsWith("<Poll>", StringComparison.Ordinal))
            .Methods.Single(m => m.Name == "MoveNext");
        Require(Calls(poll, "RefreshLoadedScenes"),
            "Each authoring poll must refresh live scenes before submitting draft edits.");
        var scenes = editor.Methods.Single(m => m.Name == "RefreshLoadedScenes");
        Require(Calls(scenes, "GetSceneAt") && Calls(scenes, "get_gameObject") && Calls(scenes, "get_isLoaded"),
        ...
        Require(common.Fields.Any(f => f.Name == "EftBattleUIScreen" && f.IsPublic && f.FieldType.FullName == "EFT.UI.EftBattleUIScreen"),
        ...
        Require(battle.Fields.Any(f => f.FieldType.FullName == "EFT.UI.BattleStancePanel")
        ...
        Require(Reads(mode.Methods.Single(m => m.Name == "UpdateHud"), "EftBattleUIScreen"),
        ...
        Require(Calls(hud.Methods.Single(m => m.Name == "Suppress"), "GetComponentsInChildren"),
            "HUD suppression must also cover independently animated child canvas groups.");
        Require(!Calls(hud.Methods.Single(m => m.Name == "SuppressObject"), "SetActive"),
            "HUD suppression must preserve native canvas lifecycle.");
        Require(Calls(mode.Methods.Single(m => m.Name == "Awake"), "add_willRenderCanvases")
        ...
        Require(!Calls(mode.Methods.Single(m => m.Name == "Update"), "Suppress")
        ...
        Console.WriteLine("Editor routes/HUD: camera placement, start default, teleport ordering and native battle panel contracts passed offline.");
    }

    private static bool Reads(MethodDefinition method, string field) => method.Body.Instructions.Any(i =>
        i.OpCode.Code is Code.Ldfld or Code.Ldflda && i.Operand is FieldReference f && f.Name == field);

    private static bool Calls(MethodDefinition method, string name) => method.Body.Instructions.Any(i =>
        i.Operand is MethodReference m && m.Name == name);
    ...
    private static bool EndsFrameAfterWalkthroughExit(MethodDefinition update)
    {
        ...
        if (((MethodReference)call.Operand).ReturnType.FullName != "System.Boolean") return false;
        var next = call.Next;
        while (next?.OpCode.Code == Code.Nop) next = next.Next;
        ...
        while (next != null && visited.Add(next))
        {
            ...
            {
                case Code.Ret: return true;
                case Code.Nop: next = next.Next; break;
                case Code.Br: case Code.Br_S: case Code.Leave: case Code.Leave_S:
                    next = (Instruction)next.Operand;
                    break;
                default: return false; // Any other work can process the same input twice.
            }
        }
        return false;
    }
