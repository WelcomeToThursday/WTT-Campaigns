...
internal static class EditorLoadingChecks
{
    internal static void Native(AssemblyDefinition native, AssemblyDefinition client)
    {
        ...
        {
            if (!valid) throw new InvalidOperationException("Editor loading: " + message);
        }
        ...
        var openMachine = ((TypeReference)open.CustomAttributes.Single(a => a.AttributeType.Name == "AsyncStateMachineAttribute")
            .ConstructorArguments[0].Value).Resolve();
        ...
        Require(clock >= 0 && clock < match && loading[clock - 1].Operand is MethodReference { Name: "get_Now", DeclaringType.Name: "DateTimeExtensions" },
        ...
        Require(screenType.Methods.Single(m => m.Name == "Show" && m.Parameters.Count == 3).Body.Instructions
            .Any(i => i.Operand is MethodReference { Name: "get_MatchingStartTime" }), "Loading screen reads the initialized matching clock.");
        Require(editor.Methods.Single(m => m.Name == "Awake").Body.Instructions
            .Any(i => i.Operand is MethodReference { Name: "Enable", DeclaringType.Name: "EditorDeployment" }), "Deployment guard is registered at startup.");

        var routine = native.MainModule.GetType("EFT.LocalGame").Methods.Single(m => m.Name == "vmethod_2");
        var machine = ((TypeReference)routine.CustomAttributes.Single(a => a.AttributeType.Name == "IteratorStateMachineAttribute")
            .ConstructorArguments[0].Value).Resolve();
        ...
        Require(output.Length == source.Count + 1 && output.Where(source.Contains).SequenceEqual(source.Cast<object>()),
            "Deployment patch preserves all native setup instructions and coroutine completion.");
        Require(output.Count(i => operandField.GetValue(i) is MethodInfo { Name: "Delay" }) == 1
        ...
        {
            var rejected = false;
            try { _ = ((IEnumerable)transpiler.Invoke(null, new object[] { input })!).Cast<object>().ToArray(); }
        ...
        foreach (var name in new[] { "Delay", "Show" })
            Require(deployment.Methods.Single(m => m.Name == name).Body.Instructions.Any(i => i.Operand is MethodReference { Name: "get_LoadingMap" }),
                "Deployment bypass must be limited to the authenticated editor map load: " + name);
        Require(deployment.Methods.Single(m => m.Name == "Show").Body.Instructions.Any(i => i.Operand is MethodReference { Name: "CloseScreen" }),
            "Skipping countdown still closes the loading controller before native MenuUI teardown.");
        Console.WriteLine("Editor loading: native timer initialization and actual deployment transpiler passed offline; setup and labels preserved, missing sites rejected.");
    }
}
