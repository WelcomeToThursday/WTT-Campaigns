    ...
    internal static void Run(string assemblyPath, Action<bool, string> check)
    {
            ...
            check(
                types[screen]
                    .Methods.Count(method =>
                        method.Name == "Show" && method.Parameters.FirstOrDefault()?.ParameterType.FullName == "EFT.Profile"
                    ) == 1,
        ...
        foreach (var methodName in new[] { "Consume", "Process" })
        {
            ...
            for (var i = 1; i < code.Count; i++)
            {
                if (
                    ...
                    && (getter.ReturnType.FullName == "System.Single" || getter.ReturnType is GenericParameter)
                )
                {
                    matches++;
                }
            }
            check(matches == (methodName == "Consume" ? 1 : 2), methodName + " consumption patch sites");
        }
        ...
        check(
            types["EFT.HealthSystem.ActiveHealthController/Effect"]
                ...
                .Body.Instructions.Any(i => i.Operand is MethodReference m && m.Name == "CreateInstance"),
        ...
        check(
            gridCompatibility.ReturnType.FullName == "System.Boolean" && gridCompatibility.Parameters.Single().Name == "item",
            "Grid restriction postfix binding"
        );
        var movePath = Method("EFT.InventoryLogic.ItemManipulator", "MovePathCheck");
        check(
            movePath.Parameters.Select(p => p.Name).SequenceEqual(new[] { "item", "to", "itemController" }),
            "Move/swap validation binding"
        );
        var addItem = types["EFT.InventoryLogic.ItemManipulator"].Methods.Single(m => m.Name == "Add" && m.Parameters.Count == 5);
        check(
            addItem
                .Parameters.Select(p => p.Name)
                .SequenceEqual(new[] { "item", "to", "itemController", "simulate", "ignoreRestrictions" }),
        ...
        check(
            transferStack.Parameters.Take(2).Select(p => p.Name).SequenceEqual(new[] { "source", "target" }),
            "Merge/transfer restriction binding"
        );
        check(types["EFT.InventoryLogic.MobContainer"].Properties.Any(p => p.Name == "isSecured"), "Native secure-container identity");
        var regenTick = Method("EFT.HealthSystem.ActiveHealthController/HealthBoost", "RegularUpdate");
        check(regenTick.Parameters.Single().Name == "deltaTime", "Regeneration prefix delta-time binding");
        ...
        check(
            effectUpdate.Body.Instructions.Any(i => i.OpCode == OpCodes.Sub)
                && effectUpdate.Body.Instructions.Any(i => i.OpCode == OpCodes.Starg_S || i.OpCode == OpCodes.Starg),
            "Native lifecycle clamps the final tick delta"
        );
        check(
            !types["EFT.HealthSystem.ActiveHealthController/HealthBoost"].Interfaces.Any(i => i.InterfaceType.Name == "IRestorable"),
            "Regeneration carrier cannot persist as an unrelated health effect"
        );
        check(
            types["EFT.HealthSystem.ActiveHealthController/HealthBoost"].Methods.Any(m => m.IsConstructor && m.IsPublic),
        ...
        check(
            types["EFT.Profile"].Interfaces.Any(i => i.InterfaceType.FullName == "EFT.InventoryLogic.IInventoryProfileInfo"),
            "Profile implements examination interface"
        );
        check(
            CallsExperienceSetter(Method("EFT.InventoryLogic.ItemManipulator", "FinishConditional"), "EFT.Profile"),
            "Client quest rewards use award wrapper"
        );
        check(
            CallsExperienceSetter(Method("EFT.ProfileUpdatesHandler", "ApplyExperience"), "EFT.ProfileInfo"),
            "Backend XP reconciliation bypasses award wrapper"
        );
        var treatment = Method("EFT.OfflineStatisticManager", "ExperienceGained");
        check(
            treatment.Parameters.Single().Name == "experience" && treatment.Parameters[0].ParameterType.FullName == "System.Single",
            "Treatment XP prefix parameter binding"
        );
        check(CallsExperienceSetter(treatment, "EFT.ProfileInfo"), "Treatment XP bypasses profile award hook to prevent double scaling");
        ...
        var speedLimit = types["EFT.MovementContext"].Methods.Single(m => m.Name == "AddStateSpeedLimit" && m.Parameters.Count == 2);
        check(
        ...
        check(firGetter.ReturnType.FullName == "System.Boolean" && !firGetter.IsStatic, "Hideout FiR getter signature");
        var suitable = types["EFT.Hideout.ItemRequirement"].Methods.Single(m => m.Name == "IsSuitableItem" && m.IsStatic);
        check(
            suitable.Body.Instructions.Any(i => i.Operand is MethodReference m && m.Name == firGetter.Name),
            "Hideout suitability reads FiR through getter"
        );
        check(
            types["EFT.Hideout.ItemRequirementPanel"]
                .Methods.Any(m =>
                    m.HasBody && m.Body.Instructions.Any(i => i.Operand is MethodReference called && called.FullName == firGetter.FullName)
                ),
            "Hideout requirement panel reads same FiR getter"
        );
        foreach (
            var method in new[] { "GetStaminaCapacityFunc", "GetHandsCapacityFunc", "GetStaminaRestorationFunc", "GetHandsRestorationFunc" }
        )
        {
