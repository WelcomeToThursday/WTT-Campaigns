...
internal static class EncounterClearanceChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        check(noSpace.SequenceEqual(line) && !AllClear(noSpace, Clear), "No valid detour preserves the blocked result for strict final validation");
        ...
        check(rounded.Length >= 3 && rounded[1] != bend[1] && AllClear(rounded, CornerClear),
            "A container bend shifts away from the padded footprint without changing endpoints");
        var unchanged = EncounterRouteClearance.Adjust(bend, _ => throw new Exception("unnecessary sample"), (_, _) => true, (_, _) => null);
        ...
        EncounterRouteClearance.Adjust(longRoute, p => { samples++; return null; }, (_, _) => false, (_, _) => bollard);
        ...
        check(Pace(1.8f) == 0 && Pace(2.5f, true) == 0 && Pace(2.8f, true) > 0,
        ...
        check(EncounterSquadSpacing.Pace(Vector3.Zero, east, new(1, 2, 0), east, true, false) == 1,
            "Bots on different floors do not hold each other");
        check(EncounterSquadSpacing.Pace(Vector3.Zero, east, new(1, 0, 2), east, true, false) == 1,
        ...
        check(EncounterSquadSpacing.Pace(Vector3.Zero, east, Vector3.Zero, east, true, false) == 0
    ...
    private static bool Intersects(Vector3 from, Vector3 to, EncounterRouteClearance.Obstacle obstacle)
    {
        ...
        foreach (var (start, direction, extent) in new[] { (a.X, delta.X, half.X), (a.Z, delta.Z, half.Z) })
        {
            ...
            {
                if (MathF.Abs(start) > extent) return false;
                continue;
            }
