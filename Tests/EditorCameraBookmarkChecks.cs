...
internal static class EditorCameraBookmarkChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        check(
            restored.TryGet("draft", "interchange", out var pose)
                && pose.Position.SequenceEqual(first.Position)
                && pose.Rotation.SequenceEqual(first.Rotation),
            "New session restores exact position and angle after persistence"
        );
        check(restored.TryGet("draft", "Woods", out pose) && pose.Position.SequenceEqual(second.Position), "Maps keep separate bookmarks");
        check(
            restored.TryGet("other-draft", "Interchange", out pose) && pose.Position.SequenceEqual(second.Position),
            "Drafts keep separate bookmarks"
        );
        restored.Save("draft", "Interchange", second);
        check(
            restored.TryGet("draft", "Interchange", out pose) && pose.Position.SequenceEqual(second.Position),
            "Most recent departure replaces the previous bookmark"
        );
        ...
        check(
            !restored.Save("draft", "Interchange", new() { Position = new[] { float.NaN, 0f, 0f }, Rotation = first.Rotation }),
            "Invalid camera position is not persisted"
        );
        check(
            !restored.Save("draft", "Interchange", new() { Position = first.Position, Rotation = new float[4] }),
            "Invalid camera orientation is not persisted"
        );
        check(
            restored.TryGet("draft", "Interchange", out pose) && pose.Position.SequenceEqual(second.Position),
            "Invalid save preserves the last valid camera"
        );
