...
internal static class MissionRuntimeChecks
{
    internal static void Run(Action<bool, string> check)
    {
        var checkpoints = new[]
        {
            new MapVolume { Id = "first" },
            new MapVolume { Id = "second" },
        };
        ...
        check(
            MissionRunRules.MatchesIdentity(run, "character", "run", "raid", "token"),
            "Mission identity accepts its exact run and encounter token"
        );
        foreach (var invalid in new[] { "", "different" })
        {
            ...
            check(
                !MissionRunRules.MatchesIdentity(run, "character", "run", "raid", invalid),
                "Mission identity rejects wrong/empty encounter token"
            );
        }
        check(
            !MissionRunRules.TryCheckpoint(run, checkpoints, "second", out _) && run.NextCheckpointIndex == 0,
            "Mission checkpoints cannot be skipped"
        );
        check(
            !MissionRunRules.TryExit(run, checkpoints, exit, "exit", out _) && !run.ExitReached,
            "Mission exit rejects an incomplete route"
        );
        check(
            MissionRunRules.TryCheckpoint(run, checkpoints, "first", out _) && run.NextCheckpointIndex == 1,
            "First checkpoint advances once"
        );
        check(
            MissionRunRules.TryCheckpoint(run, checkpoints, "first", out _) && run.NextCheckpointIndex == 1,
            "Repeated checkpoint is an idempotent no-op"
        );
        check(
            !MissionRunRules.TryCheckpoint(run, checkpoints, "unknown", out _) && run.NextCheckpointIndex == 1,
            "Unknown checkpoint cannot advance progress"
        );
        check(
            MissionRunRules.TryCheckpoint(run, checkpoints, "second", out _) && run.NextCheckpointIndex == 2,
            "Second checkpoint completes the route"
        );
        check(
            !MissionRunRules.TryExit(run, checkpoints, exit, "normal-extract", out _) && !run.ExitReached,
            "Only the authored exit can be recorded"
        );
        check(
            !MissionRunRules.IsSuccessfulExtraction(run, checkpoints, exit, "Survived", exit.Name, out _),
            "Native extraction alone cannot complete a mission"
        );
        ...
        foreach (var failure in new[] { "Killed", "MissingInAction", "Left", "Disconnected", "", "unknown" })
            check(
                !MissionRunRules.IsSuccessfulExtraction(run, checkpoints, exit, failure, exit.Name, out _),
                "Non-survival result never completes mission: " + failure
            );
        check(
            !MissionRunRules.IsSuccessfulExtraction(run, checkpoints, exit, "Survived", "normal-extract", out _),
            "Wrong native exit name cannot finalize a mission"
        );
        check(
            MissionRunRules.IsSuccessfulExtraction(run, checkpoints, exit, "Survived", exit.Name, out _),
            "Alive authored extraction completes mission"
        );
        var retry = new MissionRun
        {
            CharacterId = "character",
            RunId = "retry",
            RaidId = "new-raid",
        ...
        check(
            retry.NextCheckpointIndex == 0 && !retry.ExitReached && retry.CompletedCheckpointIds.Count == 0,
            "Retry starts without prior route state"
        );
        ...
        var source = new List<NativeItem>
        {
            ...
            new()
            {
                ...
                Template = "55818b164bdc2ddc698b456c",
        ...
        check(
            first[1].ParentId == first[0].Id && first[1].SlotId == source[1].SlotId,
            "Native mission loot preserves assembly links after identity remap"
        );
