...
internal static class ScenePreviewChecks
{
    ...
    {
        check(!PreviewMaterialPolicy.UsesOpacity("Opaque", "Custom/Bumped Specular", false, false), "Opaque prop surface masks must not cut holes in thumbnails");
        check(!PreviewMaterialPolicy.UsesOpacity("", "Custom/Container", false, false), "Untagged solid props default to opaque previews");
        check(PreviewMaterialPolicy.UsesOpacity("TransparentCutout", "Custom/Fence", false, false), "Cutout fence textures retain their holes");
        check(PreviewMaterialPolicy.UsesOpacity("", "Custom/Leaves", false, false), "Leaf shaders retain opacity even without a render tag");
