...
internal static class EditorSceneVisibilityChecks
{
    internal static void Native(AssemblyDefinition native, AssemblyDefinition client)
    {
        ...
        {
            if (!valid) throw new InvalidOperationException("Editor visibility: " + message);
        }
        ...
        Require(desired.Any(i => i.Operand is FieldReference { Name: "isGroupEnabled" })
            && desired.Any(i => i.Operand is MethodReference { Name: "Toggle" }), "Native requested state is stored separately from visible rendering.");
        var query = types["Koenigz.PerfectCulling.EFT.CullingGridVisibilityQueryResult"];
        Require(query.Methods.Single(m => m.Name == "TestAABB").Body.Instructions.Any(i => i.Operand is MethodReference { Name: "get_IsEmpty" }),
        ...
        Require(dispose.Any(i => i.Operand is MethodReference { Name: "get_IsEnabled" })
            && dispose.Any(i => i.Operand is MethodReference { Name: "Toggle" }), "Closing restores the latest native requested state.");
        Require(!compiledType.Methods.Where(m => m.HasBody).SelectMany(m => m.Body.Instructions)
        ...
        Require(environment.Methods.Single(m => m.Name == "Dispose").Body.Instructions
            .Any(i => i.Operand is MethodReference { Name: "Dispose", DeclaringType.Name: "EditorSceneVisibility" }),
        ...
        Require(switchBody.Count(i => i.Operand is MethodReference { Name: "SetComponentsEnabled", DeclaringType.Name: "CustomCullingCommon" }) == 2
        ...
        foreach (var name in new[] { "_setComponentsEnabledWorker", "_setComponentsEnabledWorker2" })
            Require(trigger.Fields.Any(f => f.Name == name && f.IsPrivate && f.FieldType.FullName == "System.Collections.IEnumerator"),
        ...
        var calls = triggerLease.Methods.Where(m => m.HasBody).SelectMany(m => m.Body.Instructions)
            .Where(i => i.Operand is MethodReference).Select(i => (MethodReference)i.Operand).ToArray();
        Require(calls.Any(m => m.Name == "StopCoroutine" && m.Parameters.Single().ParameterType.FullName == "System.Collections.IEnumerator")
            && calls.Any(m => m.Name == "IsEnabledUniversal") && calls.Any(m => m.Name == "SetEnabledUniversal")
            && calls.Any(m => m.Name == "ForceUpdate"), "Trigger lease uses exact native component and coroutine APIs and requests restoration.");
        ...
        Require(awake.Body.Instructions.Any(i => i.Operand is MethodReference { Name: "Enable", DeclaringType.Name: "EditorSceneVisibility" }),
            "Culling hooks install before map loading can inline the native methods.");

        var context = new ClientAssemblyContext(Path.GetFullPath(Path.Combine(Path.GetDirectoryName(native.MainModule.FileName)!, "../../..")), client.MainModule.FileName);
        ...
        foreach (var name in new[] { "Worker", "InverseWorker" })
            Require(compiledTrigger.GetField(name, BindingFlags.NonPublic | BindingFlags.Static)!.GetValue(null) is FieldInfo field
                && field.DeclaringType!.Name == "DisablerCullingObject" && field.FieldType == typeof(System.Collections.IEnumerator),
        ...
        try
        {
            foreach (var enabled in new[] { false, true, false, true, false })
            {
                ...
                Require((bool)test.Invoke(null, queryArgs)! == !enabled && (bool)queryArgs[0] == enabled,
                ...
                foreach (var requested in new[] { false, true })
                {
                    ...
                    Require((bool)args[1] == (requested || enabled && ReferenceEquals(target, group)), "Only registered editor scene groups bypass baked hiding.");
                    Require((bool)groupType.GetProperty("IsEnabled")!.GetValue(target)! == requested, "Rendering overrides leave native requested visibility intact.");
                }
            }
        }
        finally { editing.SetValue(null, false); active.SetValue(null, null); }
        Console.WriteLine("Editor visibility: grid/baked guards, native trigger workers, both component paths and early hook registration verified offline; state restoration tested with managed doubles.");
    }
}
