...
internal static class DraftManagementChecks
{
    public static void Run(SeasonRepository store, string directory, DraftEnvelope source, Action<bool, string> check)
    {
        ...
        check(draft.Definition.Name == "Managed draft" && draft.Definition.Id == seasonId, "Quick rename trims the name and preserves season identity");
        ...
        check(!store.Drafts().Any(d => d.Id == id) && store.Drafts(DraftStatus.Archived).Any(d => d.Id == id), "Archived drafts leave the editing list and appear in Archived");
        ...
        Reject(() => store.Publish(draft, SeasonValidator.Validate(draft.Definition)), "Archived drafts cannot be published without restoring");
        ...
        check(store.Drafts(DraftStatus.Trashed).Any(d => d.Id == id) && !store.Drafts(DraftStatus.Archived).Any(d => d.Id == id), "Trash removes a draft from Archived and keeps it recoverable");
        ...
        check(JsonConvert.SerializeObject(draft.Definition) == trashedJson && draft.LastEditedUtc == edited, "Restore preserves complete content and last edit time across restart");
        check(store.Drafts().Any(d => d.Id == id) && SeasonRepository.GameplayHash(draft.Definition) == gameplay, "Restored draft returns to editing with unchanged gameplay");
        check(store.Pack(pack).Name == publishedName && File.ReadAllBytes(publishedPath).SequenceEqual(publishedBytes), "Rename, archive and trash leave published content byte-for-byte unchanged");
        ...
        check(loaded.Status == DraftStatus.Active && loaded.LastEditedUtc?.Year == 2025, "Older drafts default to Drafts and use their file date for sorting");
        ...
        check(loaded.LastEditedUtc?.Year >= 2026 && store.Load(legacy.Id).Definition.Description == loaded.Definition.Description, "Editing an old draft persists its new metadata and content");
