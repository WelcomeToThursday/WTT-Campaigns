...
if (
    args.Length == 3
    && (
        args[0] == "--resource-hooks"
        || args[0] == "--bush-hooks"
        || args[0] == "--experience-hooks"
    )
)
{
    SeasonalPerks.Tests.ClientHookChecks.Run(
        args[1],
        args[2],
        args[0] == "--bush-hooks",
        args[0] == "--experience-hooks"
...
Check(
    seasoned.Multiplier("pmc_experience_multiplicator") == 1.25f,
    "Captured PMC experience bonus"
);
...
Check(
    ExperienceScaling.Total(int.MaxValue - 10, int.MaxValue, 1.25f) == int.MaxValue,
    "Bonus cannot overflow profile XP"
);
Check(ExperienceScaling.Award(100, 1f) == 100, "Normal PMC award stays neutral");
Check(
    new RuntimeEffects(c, new[] { "69c3da8fc0e4deb02605f3c9" }).Has("flea_market_npc_only"),
    "Captured trader-only restriction"
);
...
foreach (
    var medical in c
        .All.Single(p => p.Id == "69c3d036042c81ad9209eeae")
        .Effects[0]
        .ItemFilter!.Include!
)
    Check(
...
Check(
    c.All.Count(p => EffectSupport.UnavailableReason(p) == null) == 33,
    "33 implemented catalogue entries"
);
...
Check(
    vacuum.TraderMultiplier(therapist, "buy") == 1.2m,
    "Personality Vacuum increases purchases by 20 percent"
);
Check(
    vacuum.SkillBlocked("Charisma") && !vacuum.SkillBlocked("Strength"),
    "Personality Vacuum blocks only Charisma growth"
);
Check(
    thirdLeg.TraderMultiplier(therapist, "buy") == 0.95m,
    "Third Leg discounts Therapist purchases"
);
Check(
    thirdLeg.Multiplier("sprint_speed_multiplicator") == 0.99f,
    "Third Leg preserves captured sprint penalty"
);
Check(
    thirdLeg.TraderMultiplier("54cb50c76803fa8b248b4571", "buy") == 1m,
    "Third Leg leaves other traders alone"
);
Check(
    Selection.Validate(
        c,
        new[] { "69c3d43030f896ebef0ed357", "69ce62886e199f4bbe0ab19b" },
        new Rules(),
        unavailable
    ) != null,
...
Check(
    vacuum.TraderMultiplier(therapist, "sell") == 1m,
    "Purchase effects do not change sale proceeds"
);
Check(
    vacuum.TraderMultiplier("custom-trader", "buy") == 1m,
    "Captured trader allow-list excludes custom traders"
);
Check(TraderPricing.Scale(10000, 1.2m) == 12000, "No extra currency unit from a float multiplier");
Check(
    TraderPricing.Scale(10000, 1.14m) == 11400,
    "Stacked multiplier stays exact at integral prices"
);
Check(
    TraderPricing.Required(TraderPricing.Scale(101, 0.95m), 1) == 96,
    "Fractional unit price rounds upward at purchase"
);
Check(
    TraderPricing.Required(TraderPricing.Scale(101, 0.95m), 2) == 192,
    "Bulk payment uses native quantity rounding"
);
Check(
    TraderPricing.Required(TraderPricing.Scale(1, 1.2m), 5) == 6,
    "Barter requirement rounds after quantity multiplication"
);
var neutral = new RuntimeEffects(c, Array.Empty<string>());
Check(
    bush.BushNoiseMultiplier == 0.25f && bush.BushSlowdownMultiplier == 0.25f,
    "Captured bush multipliers"
);
Check(
...
Check(
    Math.Abs(BushInteraction.SpeedLimit(0.2f, neutral.BushSlowdownMultiplier) - 0.2f) < 0.0001f,
    "Removing Bushborne restores slowdown"
);
Check(neutral.BushNoiseMultiplier == 1f, "Removing Bushborne restores noise");
Check(
    BushInteraction.SpeedLimit(1f, 0.25f) == 1f,
    "Bushborne adds no speed above the original limit"
);
...
Check(
    neutral.HideoutRequiresFir(true) && !neutral.HideoutRequiresFir(false),
    "Removal restores both original requirement states"
);
Check(
