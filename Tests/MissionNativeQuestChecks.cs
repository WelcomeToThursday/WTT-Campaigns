...
internal static class MissionNativeQuestChecks
{
    internal static void Run(string nativePath, Action<bool, string> check)
    {
        ...
        check(
            timers.Fields.Any(f => f.Name == "_timerPanelTemplate" && f.IsPublic)
                && timers.Fields.Any(f => f.Name == "_container" && f.IsPublic),
            "Additional extracts use the native timer prefab and container"
        );
        ...
        check(
            pointType.Interfaces.Any(i => i.InterfaceType.Name == "IPhysicsTrigger"),
            "Native level extracts use physics trigger dispatch"
        );
        ...
        check(
            extraction.Properties.Single(p => p.Name == "ExfiltrationPoints").SetMethod.IsPublic,
            "Added level extracts can register without replacing native exits"
        );
        check(
            scenario
                .Methods.Single(m => m.Name == "Update")
                .Body.Instructions.Any(i => i.Operand is FieldReference f && f.Name == "ExfiltrationTime"),
            "Native scenario owns the extract countdown"
        );
