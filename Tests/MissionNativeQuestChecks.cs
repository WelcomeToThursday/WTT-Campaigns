...
internal static class MissionNativeQuestChecks
{
    internal static void Run(string nativePath, Action<bool, string> check)
    {
        ...
        check(types.ContainsKey("EFT.Quests.ConditionGlobalVariableValue"), "Installed client supports mission completion variable condition");
        check(types["EFT.Quests.ConditionGlobalVariableValue"].BaseType.FullName == "EFT.Quests.ConditionOneTarget", "Mission completion variable uses one target");
        check(types["EFT.Quests.QuestTemplate/EQuestType"].Fields.Any(f => f.Name == "Exploration"), "Installed client accepts mission test quest type");
        ...
        check(uncover.IsPublic && uncoverCalls.Contains("UncoverItem") && uncoverCalls.Contains("DiscoverItem")
        ...
        check(searchController.Methods.Single(m => m.IsConstructor).Body.Instructions
            .Any(i => i.Operand is MethodReference m && m.Name == "UncoverContent"),
            "Preview gear uses the same search initialization as native player equipment");
        var spawnCalls = world.Methods.Single(m => m.Name == "SpawnLootItem").Body.Instructions
        ...
        check(ownerConstructor.Parameters.Count == 5
        ...
        check(ownerCalls.Any(m => m.Name == "CreateItemAddress") && ownerCalls.Any(m => m.Name == "set_CurrentAddress"),
            "Native loot ownership supplies the detached root item's inventory address");
        check(world.Methods.Any(m => m.Name == "CreateStaticLoot" && m.Parameters.Count == 7
            && m.Parameters[0].ParameterType.FullName == "UnityEngine.GameObject"
            && m.ReturnType.FullName == "EFT.Interactive.LootItem"), "Mission loot uses the installed native world loot constructor");
        check(world.Methods.Any(m => m.Name == "DestroyLoot" && m.Parameters.Count == 1
            && m.Parameters[0].ParameterType.FullName == "IKillable"), "Mission rollback uses the native loot removal lifecycle");
        var factory = types.Values.Single(t => t.Name == "ObjectsFactory");
        check(factory.Methods.Any(m => m.Name == "CreateLootPrefab" && m.Parameters.Count == 3
            && m.ReturnType.FullName == "UnityEngine.GameObject"), "Mission loot uses an interactable native prefab");
        ...
        foreach (var name in new[] { "ExfiltrationPoints", "ScavExfiltrationPoints", "SecretExfiltrationPoints" })
            check(extraction.Fields.Any(f => f.Name == name) || extraction.Properties.Any(p => p.Name == name),
        ...
        var layout = new MapLayout
        {
            ...
            Checkpoints = [new() { Id = Id(), Location = "Interchange", Scene = "Test" }],
            Exit = new() { Id = Id(), Location = "Interchange", Scene = "Test" },
        };
        var prepared = MissionTestCampaign.AddMission(new SeasonDefinition { Id = Id(), BattlePassId = Id(), MapLayouts = [layout] }, layout.Id);
        ...
        check((string?)condition["target"] == prepared.Story!.Variables.Single().Id, "SPT serialization preserves mission variable as a scalar target");
        check((double?)condition["value"] == 1 && (string?)condition["compareMethod"] == ">=", "SPT serialization preserves mission completion comparison");
        var statuses = types["EFT.Quests.EQuestStatus"].Fields.Select(f => f.Name).ToHashSet();
        foreach (var section in new[] { "conditions", "rewards" })
            check(((JObject)wire[section]!).Properties().All(p => statuses.Contains(p.Name)), "Mission " + section + " only use installed client quest-status keys");
    }
