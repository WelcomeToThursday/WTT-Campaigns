    ...
    internal static void Run(Action<bool, string> check)
    {
        ...
        try
        {
            ...
            JsonConvert.DefaultSettings = () =>
                new JsonSerializerSettings { Converters = { new StoryEnumConverter(), new StringEnumConverter() } };
            const string json = """{"blackdiv":{"BotTypes":[848420,1]}}""";
            var factions = JsonConvert.DeserializeObject<Dictionary<string, Faction>>(json)!;
            check(
                (int)factions["blackdiv"].BotTypes[0] == 848420,
                "MoreBots numeric custom role survives globally discovered story converter"
            );
            ...
            foreach (
                var type in new[]
                {
                    typeof(StoryActionType),
                    typeof(StoryActionType?),
                    typeof(StoryVariableScope),
                    typeof(StoryVariableScope?),
                }
            )
            {
                ...
                try
                {
                    JsonConvert.DeserializeObject("0", type);
                }
            ...
            check(
                JsonConvert.DeserializeObject<StoryActionType>("\"AcceptQuest\"") == StoryActionType.AcceptQuest,
                "Named story actions still load"
            );
            check(
                JsonConvert.DeserializeObject<StoryVariableScope>("\"Profile\"") == StoryVariableScope.Profile,
                "Named story scopes still load"
            );
