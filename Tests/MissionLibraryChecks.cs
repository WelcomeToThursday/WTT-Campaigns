...
internal static class MissionLibraryChecks
{
    internal static void Run(SeasonRepository store, Action<bool, string> check)
    {
        ...
        check(validation.CanPublish, "Mission without a campaign or quest validates: " + string.Join("; ", validation.Issues.Select(i => i.Message)));
        ...
        var image = store.AddImage(Convert.FromBase64String("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg=="));
        ...
        check(link.Availability == MissionAvailability.FromStart && link.Revision == 1, "Campaign links start available and pin a published revision");
        ...
        check(link.Revision == 1 && JsonConvert.SerializeObject(link.Package) == untouched, "Publishing a new mission revision does not change a campaign link");
        store.UpdateMissionLink(link, secondKey);
        check(link.Revision == 2, "Authors can explicitly update a linked revision");
        var duplicateRejected = false;
        try { store.LinkMission(campaign.Definition, secondKey); } catch (InvalidOperationException) { duplicateRejected = true; }
        ...
        check(resolvedMission.Id == link.Id && resolved.MapLayouts.Any(l => l.Id == resolvedMission.LayoutId), "Runtime composition preserves link identity and resolves its own layout");
        check(resolvedMission.LayoutId != link.Package.MapLayouts[0].Id, "Linked layout identities are scoped to their campaign link");
        check(JsonConvert.SerializeObject(MissionLibrary.Resolve(campaign.Definition)) == JsonConvert.SerializeObject(resolved), "Runtime layout namespacing is deterministic");
        check(MissionLibrary.Standalone([published], null).Missions.Count == 0, "Campaign-only publication is absent from the normal character library");
        ...
        newer.Revision = 2;
        var standaloneRun = new MissionRun { ContextVersion = 3, Scope = MissionLibrary.StandaloneScope, PackageId = standalone.Id,
            PackageRevision = 1, MissionId = standalone.Missions[0].Id, Status = MissionRunStatuses.Active };
        check(MissionLibrary.Standalone([standalone, newer], null).MissionLinks.Single().Revision == 2, "Normal character library selects the latest published mission revision");
        check(MissionLibrary.Standalone([standalone, newer], standaloneRun).MissionLinks.Single().Revision == 1, "An active standalone run retains its prepared mission revision");
        newer.MissionPackage!.AllowStandalonePlay = false;
        check(MissionLibrary.Standalone([standalone, newer], null).Missions.Count == 0, "A latest revision disabling standalone access does not expose older revisions");
        check(MissionLibrary.Standalone([standalone, newer], standaloneRun).MissionLinks.Single().Revision == 1, "An already active run remains bound to its original available revision");
        standaloneRun.Scope = "another-campaign";
        check(MissionLibrary.Standalone([standalone, newer], standaloneRun).Missions.Count == 0, "A campaign run cannot authorize an old standalone revision");
        var copied = SeasonRepository.Duplicate(campaign.Definition);
        check(copied.MissionLinks[0].Id != link.Id && copied.MissionLinks[0].ContentHash == link.ContentHash
            && JsonConvert.SerializeObject(copied.MissionLinks[0].Package) == JsonConvert.SerializeObject(link.Package), "Campaign duplication remaps link identity without rewriting pinned packages");
        ...
        check(SeasonValidator.Validate(invalid).Issues.Any(i => i.Message.Contains("checksum")), "Edited pinned mission content fails checksum validation");
        campaign = store.Save(campaign);
        var campaignValidation = SeasonValidator.Validate(campaign.Definition);
        check(campaignValidation.CanPublish, "Campaign with mission link validates: " + string.Join("; ", campaignValidation.Issues.Select(i => i.Message)));
        var campaignKey = store.Publish(campaign, campaignValidation);
        var imported = store.Import(store.Export(campaignKey));
        check(imported.Definition.MissionLinks[0].Package.Missions.Count == 1 && imported.Definition.MissionLinks[0].Revision == 2,
            "Campaign export/import carries the pinned mission package");
        check(!imported.Definition.MissionLinks[0].Package.MissionPackage!.AllowStandalonePlay, "Campaign sharing does not grant standalone access");
        var used = false;
        store.MarkUsed(campaign.Definition);
        campaign.Definition.MissionLinks[0].Availability = MissionAvailability.StoryAction;
        try { store.CheckGameplay(campaign.Definition); } catch (InvalidOperationException) { used = true; }
        ...
        var story = new StoryDefinition { Chapters = [new() { Id = chapter, Visibility = new() { Type = "VariableValue", Target = variable, Value = 1 } }],
        ...
        foreach (var status in new[] { "Locked", "AvailableForStart", "Fail", "Started", "AvailableForFinish", "Success" })
        {
            ...
            check(MissionLibrary.Eligible(link, story, progress, facts) == (status is "Started" or "AvailableForFinish" or "Success"), "Accepted gate: " + status);
        ...
        MissionStore.Write(pmc, new MissionProgress { SeasonId = "campaign", CompletedMissionIds = [link.Id], UnlockedMissionIds = [link.Id] });
        check(MissionStore.Read(pmc, "campaign").CompletedMissionIds.Contains(link.Id), "Early completion survives a profile storage round trip");
        check(MissionStore.Read(pmc, MissionLibrary.StandaloneScope).CompletedMissionIds.Count == 0 && MissionStore.Read(pmc, "other-campaign").CompletedMissionIds.Count == 0,
        ...
        {
            Id = "campaign", Story = story,
            Quests = [new() { Id = quest, Conditions = new() { AvailableForFinish = [new() {
                Id = completionId, ConditionType = "GlobalVariableValue", Target = variable, Value = 1, CompareMethod = ">=" }] } }],
        };
        ...
        check(!pmc.ExtensionData.ContainsKey("wttCampaignsStory:campaign"), "Early mission completion does not auto-accept or advance an absent quest");
        pmc.Quests = [new() { StartTime = 1, StatusTimers = new(), QId = new SPTarkov.Server.Core.Models.Common.MongoId(quest), Status = SPTarkov.Server.Core.Models.Enums.QuestStatusEnum.Started }];
        MissionCompletion.Apply(pmc, completionDefinition, completionMission);
        check(pmc.Quests[0].CompletedConditions!.Contains(completionId) && pmc.Quests[0].Status == SPTarkov.Server.Core.Models.Enums.QuestStatusEnum.Started,
        ...
        var acknowledged = new MissionRun { ContextVersion = 3, Scope = "campaign", PackageId = published.Id, PackageRevision = 1,
            RunId = "run", RaidId = "raid", CharacterId = "character", MissionId = link.Id, LayoutId = "layout", ContentHash = "hash", Status = MissionRunStatuses.Active };
        var wrongScope = SeasonCompiler.Copy(acknowledged);
        wrongScope.Scope = MissionLibrary.StandaloneScope;
        var contextRejected = false;
        try { MissionAcknowledgement.Require(acknowledged, wrongScope, true); } catch (InvalidOperationException) { contextRejected = true; }
        ...
        source.Definition.Missions = [new() { Id = SeasonRepository.NewId(), Name = "Extract me", Briefing = "Extract safely.", LayoutId = source.Definition.MapLayouts[0].Id, QuestId = quest }];
        ...
        check(extracted.Definition.Missions[0].QuestId == "" && extracted.Definition.Missions[0].Id == source.Definition.Missions[0].Id,
