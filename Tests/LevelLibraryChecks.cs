...
internal static class LevelLibraryChecks
{
    internal static void Run(SeasonRepository store, Action<bool, string> check)
    {
        ...
        check(level.Definition.Missions.Count == 0 && level.Definition.Story == null && layout.Start == null && layout.Encounters.Count == 0,
        ...
        check(validation.CanPublish, "Level backing storage remains compatible with existing publication: " + string.Join("; ", validation.Issues.Select(i => i.Message)));
        ...
        campaign.Definition.MapLayouts.Add(new() { Id = new string('a', 24), Name = layout.Name, Location = layout.Location, ApplyInNormalRaids = true });
        campaign.Definition.MapLayouts.Add(new() { Id = new string('b', 24), Name = "Legacy mission", Location = "woods", ApplyInNormalRaids = true });
        ...
        check(levels.Count == 2 && levels.All(l => l.Name == layout.Name), "Global level library includes old and new levels but excludes legacy missions");
        ...
        check(EditorContentRules.Levels(new[] { (campaign.Id, linked) }).Count == 1,
            "Retained campaign layouts owned by an extracted mission link stay out of Levels");
        check(EditorContentRules.Mode(linked, linked.MapLayouts[1].Id) == EditorContentMode.Mission,
            "Extracting a campaign mission cannot downgrade its old layout to a level");
        check(WTT.Campaigns.Shared.Spatial.MapLayerRules.ForCharacter(new[] { linked }, "", "woods") == null,
        ...
        check(EditorContentRules.Levels(new[] { ("mission", missionPackage) }).Count == 0,
            "Independent and campaign-linked mission packages are never offered as levels");
        var request = JsonConvert.DeserializeObject<EditorSessionRequest>(JsonConvert.SerializeObject(new EditorSessionRequest { Name = "Factory routes", Location = "factory4_day" }))!;
        check(request.Name == "Factory routes" && request.Version == 2, "Level creation fields are additive without changing protocol gates");
        var beforeInvalid = store.Drafts().Count;
        foreach (var invalid in new[] { ("", "woods"), ("Level", ""), ("Level", "hideout") })
        {
            try { store.CreateLevel(invalid.Item1, invalid.Item2); check(false, "Invalid level must be rejected"); }
            catch (InvalidOperationException) { check(store.Drafts().Count == beforeInvalid, "Invalid level creation writes no draft"); }
        }
    }
}
