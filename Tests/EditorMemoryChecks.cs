...
internal static class EditorMemoryChecks
{
    internal static void Run(Action<bool, string> check)
    {
        foreach (var initial in Enum.GetValues<GarbageCollector.Mode>())
        {
            ...
            check(policy.Transition(false, initial) == null && policy.Filter(initial) == initial,
                "Normal gameplay retains its GC mode: " + initial);
            check(policy.Transition(true, initial) == GarbageCollector.Mode.Enabled,
                "Editor maps enable collection from " + initial);
            for (var i = 0; i < 1000; i++)
                if (policy.Transition(true, GarbageCollector.Mode.Enabled) != null) throw new Exception("Repeated GC mode write");
            check(policy.Transition(false, GarbageCollector.Mode.Enabled) == initial,
            ...
            check(policy.Filter(GarbageCollector.Mode.Disabled) == GarbageCollector.Mode.Enabled,
            ...
            check(policy.Transition(false, GarbageCollector.Mode.Enabled) == GarbageCollector.Mode.Enabled,
            ...
            check(policy.Transition(false, GarbageCollector.Mode.Enabled) == GarbageCollector.Mode.Manual,
    ...
    internal static void Native(AssemblyDefinition native, AssemblyDefinition client)
    {
        ...
        if (!wrapper.IsPublic || !wrapper.IsStatic || wrapper.Parameters.Count != 1 || wrapper.Parameters[0].ParameterType.FullName != "System.Boolean")
        ...
        var calls = memory.Methods.Where(m => m.HasBody).SelectMany(m => m.Body.Instructions).Select(i => i.Operand).OfType<MethodReference>().ToArray();
        ...
        var syncCalls = environment.Methods.Single(m => m.Name == "Sync").Body.Instructions.Select(i => i.Operand).OfType<MethodReference>().ToArray();
        if (syncCalls.Any(m => m.DeclaringType.Name == "Time")) throw new Exception("Terrain discovery must not poll on a timer.");
        foreach (var (method, call) in new[] { (".ctor", "add_sceneLoaded"), ("Dispose", "remove_sceneLoaded") })
            if (!environment.Methods.Single(m => m.Name == method).Body.Instructions.Any(i => i.Operand is MethodReference m && m.Name == call))
                throw new Exception("Terrain discovery must follow scene lifecycle: " + call);
        Console.WriteLine("Editor memory: native GC wrapper, no forced collections, and scene-triggered terrain discovery verified offline.");
    }
}
