...
internal static class EditorTriggerChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        check(renderer.Enabled && inverse.Enabled && enabled.Enabled && culled.activeSelf,
            "Trigger visibility reveals main and inverse-independent components plus culled objects");
        check(!hidden.activeSelf && !untouched.activeSelf,
        ...
        visibility.Observe(owner);
        check(added.Enabled && visibility.ComponentCount == 4 && visibility.RevealedCount == 4,
        ...
        visibility.Dispose();
        check(!renderer.Enabled && !inverse.Enabled && !added.Enabled && enabled.Enabled,
            "Walkthrough restores original enabled states including targets shared by switches");
        check(!culled.activeSelf && !editedLater.activeSelf && !hidden.activeSelf,
            "Teardown preserves removals made before and during editing");
        check(owner.Refreshed == 1 && shared.Refreshed == 1 && visibility.SwitchCount == 0,
