...
internal static class MissionTestCampaign
{
    internal static void Prepare(string source, string layoutId, string output)
    {
        ...
        var draft = JsonConvert.DeserializeObject<DraftEnvelope>(Encoding.UTF8.GetString(sourceBytes).TrimStart('\uFEFF'))
        ...
        if (!validation.CanPublish)
            throw new InvalidOperationException(string.Join(Environment.NewLine, validation.Issues.Where(i => i.Severity == "error").Select(i => i.Path + ": " + i.Message)));
        ...
        File.WriteAllText(output + ".manifest.json", JsonConvert.SerializeObject(new
        {
    ...
    internal static void Install(string preparedPath, string backupRoot)
    {
        ...
        if (Path.GetFileName(destination) != current.Id + ".json" || Path.GetFileName(Path.GetDirectoryName(destination)) != "drafts"
            || prepared.Id != current.Id || prepared.Definition.Id != current.Definition.Id || prepared.Revision != current.Revision + 1)
    ...
    internal static SeasonDefinition AddMission(SeasonDefinition source, string layoutId)
    {
        ...
        foreach (var encounter in layout.Encounters.Where(e => e.Trigger.Type == MapEncounterTrigger.Event && e.Trigger.EventId == "manual"))
        ...
        var briefing = $"Deploy to {layout.Location}, reach the {layout.Checkpoints.Count} route checkpoints in order, and leave through the mission exit. Hostile Scavs may be avoided. The mission remains available for replay.";
        var quest = new NativeQuest
        {
            ...
            Conditions = new()
            {
                AvailableForStart = [new() { Id = SeasonRepository.NewId(), ConditionType = "Level", Value = 1, CompareMethod = ">=", DynamicLocale = false }],
            ...
            Rewards = new() { ["Started"] = [], ["Success"] = [], ["Fail"] = [] },
        ...
        foreach (var key in new[] { quest.StartedMessageText, quest.SuccessMessageText, quest.FailMessageText, quest.AcceptPlayerMessage, quest.CompletePlayerMessage, quest.DeclinePlayerMessage })
            quest.Localization["en"][key] = briefing;
        season.Quests.Add(quest);
        season.Story ??= new();
        season.Story.Chapters.Add(new() { Id = chapterId, Name = "Mission training", Order = season.Story.Chapters.Count });
        season.Story.Variables.Add(new() { Id = variableId, Scope = StoryVariableScope.Profile, InitialValue = 0 });
        season.Story.Quests.Add(new() { QuestId = questId, ChapterId = chapterId, Main = true, AutoStart = false, AutoComplete = false });
        season.Missions.Add(new MissionDefinition
        {
    ...
    internal static void Run(Action<bool, string> check)
    {
        ...
        {
            Id = Id(), Name = "Test", Location = "Interchange",
            Start = new() { Id = Id(), Location = "Interchange", Scene = "Test" },
            Checkpoints = [new() { Id = Id(), Location = "Interchange", Scene = "Test" }, new() { Id = Id(), Location = "Interchange", Scene = "Test" }],
            Exit = new() { Id = Id(), Location = "Interchange", Scene = "Test" },
        };
        var original = new SeasonDefinition { Id = Id(), BattlePassId = Id(), Name = "New campaign", MapLayouts = [layout] };
        ...
        check(prepared.Id == original.Id && prepared.MapLayouts[0].Id == layout.Id, "Mission setup preserves campaign and layout identities");
        check(JToken.DeepEquals(JToken.FromObject(layout), JToken.FromObject(prepared.MapLayouts[0])), "Mission setup preserves route geometry");
        var mission = prepared.Missions.Single();
        var quest = prepared.Quests.Single(q => q.Id == mission.QuestId);
        check(quest.TraderId == "54cb50c76803fa8b248b4571" && quest.Rewards.Values.All(r => r.Count == 0), "Test quest uses Prapor without extra rewards");
        ...
        check(!prepared.Story!.Quests.Single().AutoStart && !prepared.Story.Quests.Single().AutoComplete, "Test quest requires acceptance and turn-in");
        check(JToken.DeepEquals(JToken.FromObject(prepared), JToken.FromObject(AddMission(prepared, layout.Id))), "Mission setup is idempotent");
        var encounterSource = SeasonCompiler.Copy(original);
        encounterSource.MapLayouts[0].Encounters.Add(new()
        {
            Id = Id(), Trigger = new() { Type = MapEncounterTrigger.Event, EventId = "manual" },
            Waves = [new() { Id = Id(), Roster = [new() { Id = Id(), Role = "assault", Difficulty = "hard", Count = 3 }] }],
        });
        ...
        check(authored.Trigger.Type == MapEncounterTrigger.MissionStart && authored.Trigger.EventId.Length == 0,
            "Test setup replaces the unconnected manual event with mission start");
        check(JToken.DeepEquals(JToken.FromObject(authored.Waves), JToken.FromObject(encounterSource.MapLayouts[0].Encounters[0].Waves)),
        ...
        check(JToken.DeepEquals(JToken.FromObject(encounterPrepared.MapLayouts), JToken.FromObject(encounterSource.MapLayouts)),
