...
internal static class EditorSessionChecks
{
    internal static async Task Run(Action<bool, string> check)
    {
        ...
        try
        {
            ...
            var routeDefinition = new SeasonDefinition
            {
                Id = "patrol",
                Name = "Patrol drafts",
                FormatVersion = 6,
            ...
            routeDefinition.MapLayouts.Add(
                new()
                {
                    Id = "layout",
                    PatrolRoutes =
            ...
            foreach (
                var operation in new Action<MapPatrolRoute>[]
                {
                    r =>
                        MapPatrolRouteEditing.Insert(
                            r,
                            1,
                            new()
                            {
            ...
            )
            {
                ...
                check(
                    beforeRoute != afterRoute && routeSession.ContentVersion == revision + 1,
                    "A waypoint operation commits one editor revision"
                );
                routeSession.Undo(false);
                check(
                    JsonConvert.SerializeObject(routeSession.Definition!.MapLayouts[0].PatrolRoutes[0]) == beforeRoute,
                    "One undo restores waypoint order, IDs, positions and waits"
                );
                routeSession.Undo(true);
                check(
                    JsonConvert.SerializeObject(routeSession.Definition!.MapLayouts[0].PatrolRoutes[0]) == afterRoute,
