...
internal static class EditorOpenChecks
{
    ...
    internal static void Client(AssemblyDefinition assembly, Action<bool, string> check)
    {
        ...
        var cameraOpenCalls = editor.Methods.Single(m => m.Name == "Open").Body.Instructions
            .Where(i => i.Operand is MethodReference)
            .Select(i => ((MethodReference)i.Operand).Name).ToList();
        var persistedCamera = cameraOpenCalls.IndexOf("RestoreCameraBookmark");
        check(persistedCamera >= 0 && persistedCamera < cameraOpenCalls.IndexOf("RestoreWalkCamera"),
            "New sessions restore persisted camera before the same-raid preview bookmark override");
        check(editor.Methods.Single(m => m.Name == "Close").Body.Instructions
                .Any(i => i.Operand is MethodReference m && m.Name == "SaveCameraBookmark"),
