...
internal static class SceneMovementNativeChecks
{
    internal static void Run(Dictionary<string, TypeDefinition> types)
    {
        ...
        bool Calls(string type, string method, string called) => types[type].Methods
            .Where(m => m.Name == method && m.HasBody)
        ...
        Require(types["TreeInteractivePart"].Fields.Any(f => f.Name == "InteractivePart" && f.FieldType.FullName == "UnityEngine.GameObject"),
            "tree ownership reference is available");
        Require(types["EFT.Interactive.Trunk"].BaseType.FullName == "EFT.Interactive.WorldInteractiveObject", "trunk uses native interaction frame");
        Require(Calls("EFT.Interactive.LootableContainer", "SmoothDoorOpenCoroutine", "set_localPosition")
            || types["EFT.Interactive.LootableContainer"].NestedTypes.Any(t => t.Methods.Any(m => m.HasBody
                && m.Body.Instructions.Any(i => i.Operand is MethodReference r && r.Name == "set_localPosition"))),
            "container animation retains local drawer positions");
        Require(Calls("DoorHandle", "DefPos", "set_localPosition") && Calls("DoorHandle", "DefPos", "set_localRotation"),
        ...
        Require(Calls("EFT.Interactive.WorldInteractiveObject", "set_CurrentAngle", "UpdateInteractableTransform"),
