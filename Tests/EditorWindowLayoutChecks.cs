...
internal static class EditorWindowLayoutChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        {
            new EditorWindowPlacement { Id = "Library", Width = 440, Height = 620, X = -.35f, Y = .1f, Visible = true },
            new EditorWindowPlacement { Id = "Inspector", Width = 99999, Height = 99999, X = 99, Y = -99 },
            new EditorWindowPlacement { Id = "Controls", Width = -1, Height = -1, X = float.NaN, Y = float.PositiveInfinity },
        })
        {
            ...
            check(top <= height / 2f - 91.9f && top - fitted.Height >= -height / 2f + 39.9f, "Restored window keeps its title below the toolbar and its base on screen");
            ...
            check(Math.Abs(again.X - fitted.X) < .0001 && Math.Abs(again.Y - fitted.Y) < .0001, "Layout restore is stable across repeated display checks");
        }
        var layout = new EditorWindowLayout { Windows = new[] { new EditorWindowPlacement { Id = "Library", Width = 520, Height = 480, Visible = true, X = -.2f } } };
        var json = JsonConvert.SerializeObject(layout);
        var restored = JsonConvert.DeserializeObject<EditorWindowLayout>(json)!;
        check(restored.Version == 1 && restored.Windows[0].Width == 520 && restored.Windows[0].Visible, "Local layout survives serialization between launches");
