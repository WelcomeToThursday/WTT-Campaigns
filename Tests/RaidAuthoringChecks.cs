...
internal static class RaidAuthoringChecks
{
    public static void Run(SeasonRepository repository, Action<bool, string> check)
    {
        ...
        var request = new AuthoringRequest { ClientId = Guid.NewGuid().ToString("N"), RaidId = Guid.NewGuid().ToString("N"), Location = "woods", Enabled = true };
        var response = service.Poll("account", "character", request);
        check(response.Definition == null && service.Clients().Count == 1, "Authoring opt-in advertises presence without granting draft access");
        ...
        request.DraftId = response.DraftId; request.Grant = response.Grant; request.Revision = response.Revision;
        ...
        var zone = new SeasonZone { Id = SeasonRepository.NewId(), Name = "Camp", Location = "woods", Scene = "woods_main" };
        request.Definition = SeasonCompiler.Copy(response.Definition!);
        request.Definition.Zones.Add(zone); request.Definition.Name = "Client cannot overwrite unrelated settings";
        request.OperationId = Guid.NewGuid().ToString("N");
        response = service.Submit("account", "character", request);
        check(response.Definition!.Zones.Count == 1 && response.Definition.Name == draft.Definition.Name && response.Definition.FormatVersion == 2, "Spatial submission saves only authoring fields and promotes the pack format");
        check(service.Submit("account", "character", request).Revision == response.Revision, "Duplicate authoring operation returns its original receipt");
        ...
        var web = SeasonCompiler.Copy(baseline); web.Description = "Edited in browser";
        service.Save(draft.Id, baseline, web);
        var client = SeasonCompiler.Copy(baseline); client.Zones[0].Radius = 9;
        var merged = service.Save(draft.Id, baseline, client);
        check(merged.Conflicts.Count == 0 && merged.Definition!.Description == web.Description && merged.Definition.Zones[0].Radius == 9, "Independent browser and client edits merge: " + Newtonsoft.Json.JsonConvert.SerializeObject(new { merged.Conflicts, merged.Definition!.Description, merged.Definition.Zones }));
        var local = SeasonCompiler.Copy(merged.Definition!); var remote = SeasonCompiler.Copy(merged.Definition!);
        local.Zones[0].Name = "Local"; remote.Zones[0].Name = "Remote";
        service.Save(draft.Id, merged.Definition!, remote);
        var conflict = service.Save(draft.Id, merged.Definition!, local);
        check(conflict.Conflicts.Count == 1 && conflict.Candidate!.Zones[0].Name == "Local" && conflict.RemoteCandidate!.Zones[0].Name == "Remote", "Conflicts return both explicit choices without overwriting the draft");
        ...
        check(resolved.Conflicts.Count == 0 && resolved.Definition!.Zones[0].Name == "Local", "Choosing a conflict version uses the current server baseline");

        var invalid = SeasonCompiler.Copy(resolved.Definition!); invalid.Zones[0].Size.X = float.NaN;
        Reject(() => service.Save(draft.Id, resolved.Definition!, invalid), "Non-finite geometry is rejected");
        invalid = SeasonCompiler.Copy(resolved.Definition!); invalid.Zones[0].Radius = 0;
        ...
        service.Request(draft.Id, new() { Tool = "Zone", TargetKind = "Condition", TargetId = condition.Id });
        response = service.Poll("account", "character", request);
        var task = response.Tasks.Single();
        request.TaskId = task.Id; request.TaskStatus = "Completed"; request.ResultId = zone.Id;
        request.Definition = repository.Load(draft.Id).Definition; request.OperationId = Guid.NewGuid().ToString("N");
        response = service.Submit("account", "character", request);
        var assigned = SpatialRules.Conditions(response.Definition!).Single(c => c.Id == condition.Id);
        check(assigned.Target?.Values.Single() == zone.Id && assigned.ZoneId == null, "VisitPlace captures populate the native target field");
        check(service.Clients().Single().Tasks.Single().Status == "Completed", "Successful capture completion is acknowledged");
        var removed = SeasonCompiler.Copy(response.Definition!); removed.Zones.Clear();
        Reject(() => service.Save(draft.Id, response.Definition!, removed), "Referenced zones cannot be deleted");
        var inZone = NativeQuestAuthoring.Condition("InZone"); SpatialRules.Assign(inZone, zone.Id);
        check(inZone.ZoneIds?.Single() == zone.Id && inZone.ZoneId == null, "InZone captures populate native zoneIds");
        var leave = NativeQuestAuthoring.Condition("LeaveItemAtLocation"); SpatialRules.Assign(leave, zone.Id);
        ...
        check(copy.Zones[0].Id != zone.Id && SpatialRules.Conditions(copy).Any(c => SpatialRules.References(c).Contains(copy.Zones[0].Id)), "Season duplication replaces zones and repairs native references");
        var packDraft = repository.Create(true, resolved.Definition!);
        var asset = repository.AddImage(Convert.FromBase64String("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg=="));
        ...
        check(repository.Pack(pack).Zones.Count == 1 && repository.Pack(pack).FormatVersion == 2, "Spatial pack export and reload preserve format 2");
        ...
        request.RaidId = Guid.NewGuid().ToString("N"); request.Definition = null;
        ...
        {
            try { action(); } catch (Exception e) when (e is InvalidOperationException or InvalidDataException) { check(true, name); return; }
            check(false, name);
        }
    }
}
