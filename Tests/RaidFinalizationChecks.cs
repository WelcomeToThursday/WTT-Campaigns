    ...
    internal static void Run(Action<bool, string> check)
    {
        foreach (var failingStage in new[] { "story", "mission", "clear" })
        {
            ...
            Task Attempt()
            {
                var lease = new Lease();
                leases.Add(lease);
                if (RaidFinalization.RequiresNative(hub, mission))
                    nativeApplications++;
                return RaidFinalization.Complete(
                    Task.CompletedTask,
                    lease,
                    () =>
                    {
            ...
            try
            {
                ...
                check(false, "Fault injection must fail initial finalization");
            }
            catch (IOException)
            {
                check(leases.All(l => l.Released), "Finalization releases the character lease after " + failingStage + " failure");
            }
            check(hub && !cleared, "Committed native/hub receipt survives " + failingStage + " failure");
            Attempt().GetAwaiter().GetResult();
            Attempt().GetAwaiter().GetResult();
            check(
                nativeApplications == 1,
                "Finalization retry does not repeat native inventory or experience after " + failingStage + " failure"
            );
            check(
                missionCompletions == 1 && cleared && leases.All(l => l.Released),
                "Finalization recovers completion and raid cleanup once after " + failingStage + " failure"
            );
        }
