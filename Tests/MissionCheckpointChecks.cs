...
internal static class MissionCheckpointChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        check(
            rehearsal.LayoutId == testLayout.Id
                && rehearsal.CheckpointRetries
                && rehearsal.Objectives.Count == 0
                && rehearsal.Events.Count == 0
                && rehearsal.Requirements.Count == 0
                && rehearsal.QuestId.Length == 0,
            "Layout checkpoint rehearsal needs no linked mission and enables retries without objectives or quest rewards"
        );
        check(
            testLayout.Id == "unlinked-layout" && testLayout.Name == "Route rehearsal",
            "Checkpoint rehearsal preserves the authored layout"
        );
        var layout = new MapLayout
        {
            Checkpoints = [new() { Id = "cp" }],
        ...
        check(
            restored.AttemptGeneration == 2 && restored.Restoring && restored.Logic.Time == 0 && restored.Logic.Failure.Length == 0,
        ...
        Throws(
            ...
            "Restore cannot accept observations"
        );
        ...
        check(
            !restored.Restoring && restored.Logic.FiredRules.SetEquals(["start"]) && restored.Logic.Timers["clock"] == 10,
            "Committed start retry replays mission-start actions once"
        );
        Throws(
            () => start.CommitRestore(mission, layout, restored, new Dictionary<string, string>()),
            "Restore commit cannot dispatch actions twice"
        );
        for (var generation = 3; generation <= 6; generation++)
        {
            ...
            start.CommitRestore(mission, layout, restored, new Dictionary<string, string>());
            check(
                restored.AttemptGeneration == generation && restored.Logic.Time == 0 && restored.Logic.Timers["clock"] == 10,
        ...
        MissionAcknowledgement.RequireRestore(
            restored,
            JsonConvert.DeserializeObject<MissionRun>(JsonConvert.SerializeObject(retry)),
            true,
            preparing: true
        );
        check(
            !restored.ExitReached && retry.AttemptGeneration == restored.AttemptGeneration + 1,
            "Unfinished route checkpoint retry survives the response JSON boundary and acknowledges the next attempt"
        );
        check(
            retry.CheckpointId == "cp" && retry.NextCheckpointIndex == 1 && !retry.Logic.FiredRules.Contains("checkpoint"),
            "Checkpoint captures route acceptance before checkpoint actions"
        );
        check(
            !retry.Logic.Actors["living"].Dead && retry.Logic.Actors["dead"].Dead && retry.Logic.Time == 3,
        ...
        check(
            JsonConvert.SerializeObject(retry) == before && retry.Restoring,
            "Failed restore validation cannot mutate or release the run"
        );
        Throws(
            () => checkpoint.CommitRestore(mission, layout, retry, new Dictionary<string, string> { ["living"] = "dead" }),
        ...
        check(
            !MissionRunRules.IsSuccessfulExtraction(retry, layout.Checkpoints, layout.Exit, "Survived", "Exit", out _),
        ...
        check(
            retry.Logic.Actors.ContainsKey("fresh")
                && !retry.Logic.Actors.ContainsKey("living")
                && !retry.Logic.Actors.ContainsKey("pending")
                && retry.Logic.Actors["dead"].Dead
                && retry.Logic.Zones["cp"].Occupants.SequenceEqual(["fresh"]),
            "Restore remaps living actors and occupancy, preserves confirmed deaths, and discards unfinished generation"
        );
        check(
            retry.Logic.Timers["after"] == 8 && retry.Logic.Timers["clock"] == 10,
            "Checkpoint actions replay with restored timer deadlines"
        );
        Throws(
            () =>
                checkpoint.BeginRestore(
                    new()
                    {
                        RunId = "other",
                        RaidId = "raid",
        ...
        Throws(
            () => MissionTransaction.TryReplayReceipt(progress, "op", "input", out _),
            "Retired-attempt receipt cannot acknowledge a restored attempt"
        );
        MissionAcknowledgement.Require(retry, JsonConvert.DeserializeObject<MissionRun>(JsonConvert.SerializeObject(retry)), true);
        Throws(
            () => MissionAcknowledgement.Require(retry, retry, false),
            "Uncommitted observation response cannot clear pending observations"
        );
        ...
        check(
            newWaves.ReadyWaves(100).Count == 0 && newWaves.ReadyWaves(101).SequenceEqual([0]),
            "Wave delay keeps its remaining duration after rewind"
        );
        ...
        check(
            newWaves.ReadyWaves(303).Count == 0 && newWaves.ReadyWaves(304).SequenceEqual([1]),
        ...
        check(
            patrol.TargetWaypointIndex == 1
                && patrol.Status == PatrolRuntimeStatus.Waiting
                && patrol.Capture(501).WaitRemaining == 1
                && patrol.Capture(501).Direction == -1,
