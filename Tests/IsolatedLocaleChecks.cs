...
internal static class IsolatedLocaleChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        var next = registry.Install(
            "en",
            english,
            new Dictionary<string, string> { ["shared"] = "New test", ["new"] = "Replacement item" }
        );
        old.Dispose();
        check(
            english["shared"] == "New test" && english["new"] == "Replacement item",
            "Retiring a reset's old snapshot preserves the replacement locale"
        );
        next.Dispose();
        check(
            english.Count == original.Count && english["shared"] == original["shared"],
            "Ending a reset restores pre-test locale and removes temporary keys"
        );
        old.Dispose();
        next.Dispose();
