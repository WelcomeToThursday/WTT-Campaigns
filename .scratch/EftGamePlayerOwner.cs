...
public class EftGamePlayerOwner : CommonPlayerOwner<IEftSession>
{
	...
	public static TPlayerOwner Create<TPlayerOwner>(Player player, IInputTree inputTree, InsuranceCompany insurance, IEftSession session, GameUI gameUI, GameDateTime gameDateTime, [CanBeNull] LocationSettings.Location location) where TPlayerOwner : EftGamePlayerOwner
	{
	...
	public static EftGamePlayerOwner Create(Player player, IInputTree inputTree, InsuranceCompany insurance, IEftSession session, GameUI gameUI, GameDateTime gameDateTime, [CanBeNull] LocationSettings.Location location)
	{
	...
	public override bool TranslateInventoryScreenInput(ECommand command)
	{
		...
		}, base.Player.HealthController, base.Player.InventoryController, base.Player.QuestController, base.Player.AchievementsController, base.Player.PrestigeController, null, EInventoryTab.Unchanged);
		return true;
	}
	...
	public override void ShowInventoryScreenLoot(CompoundItem loot, Action callback, bool isFakeContainer = false)
	{
		...
		ShowInventoryScreen(delegate
		{
			base.Player.SetInventoryOpened(opened: false);
			callback();
		}, base.Player.HealthController, base.Player.InventoryController, base.Player.QuestController, base.Player.AchievementsController, base.Player.PrestigeController, loot, EInventoryTab.Gear, isFakeContainer);
	}

	public virtual void ShowInventoryScreen(Action exitAction, IHealthController healthController, InventoryController controller, QuestController questController, AchievementsController achievementsController, PrestigeController prestigeController, [CanBeNull] CompoundItem lootItem, EInventoryTab tab, bool showAsGridContent = false)
	{
		if (!EftScreenManager.Instance.CheckCurrentScreen(EEftScreenType.BattleUI))
		{
		...
		InventoryScreen.RaidInventoryScreenController inventoryScreenController = new InventoryScreen.RaidInventoryScreenController(Session, base.Player.Profile, healthController, controller, questController, achievementsController, prestigeController, lootItem, tab, showAsGridContent, viewType);
