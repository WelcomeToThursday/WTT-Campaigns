...
{
    private static readonly FieldInfo Duration = typeof(BetterAudio).GetField("float_1", BindingFlags.Instance | BindingFlags.NonPublic)
        ?? throw new MissingFieldException("BetterAudio", "float_1");
    private static readonly FieldInfo Deadline = typeof(BetterAudio).GetField("float_2", BindingFlags.Instance | BindingFlags.NonPublic)
        ?? throw new MissingFieldException("BetterAudio", "float_2");
    private readonly float _duration, _remaining;
    ...
    internal MissionAudioSnapshot(Player player)
    {
        ...
        if (!Singleton<BetterAudio>.Instantiated) return;
