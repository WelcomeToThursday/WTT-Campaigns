...
internal sealed class MissionAccountingSnapshot
{
    ...
    internal MissionAccountingSnapshot(Player player)
    {
        ...
        var roots = new List<object>
        {
            ...
            player.Profile.QuestsData,
            player.Profile.TaskConditionCounters,
            player.StatisticsManager,
            player,
            player.InventoryController,
            player.SearchController,
        };
        if (player.QuestController != null)
            roots.Add(player.QuestController);
    ...
    {
        if (
            ReferenceEquals(target, _player)
            || ReferenceEquals(target, _player.InventoryController)
            || ReferenceEquals(target, _player.SearchController)
        )
    ...
    {
        if (value.GetType().Name.Contains("Disposable") && value.GetType().Assembly == typeof(Player).Assembly)
            return true;
    ...
    internal void Restore(Player player)
    {
        if (!ReferenceEquals(player, _player))
            throw new InvalidOperationException("Checkpoint accounting belongs to another player.");
        _state.Restore();
