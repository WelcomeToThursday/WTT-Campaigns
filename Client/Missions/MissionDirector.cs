...
internal sealed class MissionDirector : IDisposable
{
    ...
    internal void BindInteractions(IEnumerable<(string Id, WorldInteractiveObject Object)> interactions)
    {
        foreach (var (id, target) in interactions)
            if (target) _interactions[id] = (target, target.DoorState);
    }
    ...
    {
        if (_pausedAt.HasValue) _started = UnityEngine.Time.realtimeSinceStartup - _pausedAt.Value;
        _pausedAt = null;
    }
    internal void Restore(MissionLogicState state)
    {
        ...
        _pending.Clear(); _inside.Clear(); _samples.Clear(); _activated.Clear(); Actors.Clear();
        foreach (var id in state.ActivatedEncounters) _activated.Add(id);
        foreach (var zone in state.Zones) _inside[zone.Key] = zone.Value.PlayerInside;
    }

    internal MissionDirector(MapLayout layout, Player player, EncounterPreviewRuntime encounters)
    {
        _layout = layout; _player = player; _encounters = encounters;
    ...
    {
        if (_pending.Count >= 4096) throw new InvalidOperationException("Mission observations are not being acknowledged.");
        signal.Time = Time;
    ...
    internal void Tick()
    {
        ...
        if (UnityEngine.Time.realtimeSinceStartup < _nextSample) return;
        _nextSample = UnityEngine.Time.realtimeSinceStartup + .5f;
        ObserveInteractions();
        SampleVolumes();
        if (Time - State.Time >= 1 && (State.Timers.Count > 0 || State.Objectives.Values.AsValueEnumerable().Any(o => o.Status == "Active")))
    ...
    internal void ObserveInteractions()
    {
        foreach (var pair in _interactions.AsValueEnumerable().ToArray())
        {
            ...
            if (!target || target.DoorState == pair.Value.State) continue;
    ...
    {
        foreach (var volume in _layout.Checkpoints.AsValueEnumerable().Concat(_layout.Exit == null ? Array.Empty<MapVolume>() : new[] { _layout.Exit }))
        {
            var inside = EncounterNavigation.Contains(volume, _player.Transform.position);
            if (!_inside.TryGetValue(volume.Id, out var previous)) previous = false;
            if (inside != previous) Observe(new MissionSignal { Kind = inside ? MissionSignals.Enter : MissionSignals.Leave, TargetId = volume.Id });
            ...
            {
                Observe(new MissionSignal { Kind = MissionSignals.Sample, TargetId = volume.Id, PlayerInside = inside, Occupants = occupants });
    ...
    internal List<MissionSignal> Take()
    {
        ...
        while (_pending.Count > 0 && signals.Count < 128) signals.Add(_pending.Dequeue());
        return signals;
    }
    ...
    internal void Accept(MissionLogicState state)
    {
        ...
        foreach (var id in state.ActivatedEncounters)
            if (_activated.Add(id)) _encounters.ActivateEncounter(id);
    }

    public void Dispose() { _encounters.Signal -= Observe; _encounters.ActorRegistered -= Register; _pending.Clear(); }
}
