...
internal sealed class MissionInventorySnapshot
{
    ...
    internal MissionInventorySnapshot(Player player)
    {
        ...
        foreach (EquipmentSlot slot in Enum.GetValues(typeof(EquipmentSlot)))
        {
            ...
            if (native != null)
                _equipment.Add(
                    slot,
                    native.ContainedItem == null ? null : ItemBinarySerializer.SerializeItem(native.ContainedItem, player.SearchController)
                );
        }
        ...
        if (quest != null)
            foreach (var item in quest.Grid.Items)
            {
                ...
                _questItems.Add(
                    (
                        ItemBinarySerializer.SerializeItem(item, player.SearchController),
                        new LocationInGrid(position.x, position.y, position.r)
                    )
                );
            }
        foreach (var pair in player.InventoryController.FastAccess.BoundItems)
            _bindings.Add(pair.Key, pair.Value.Id);
    ...
    internal static async Task SettleHands(Player player, CancellationToken token)
    {
        ...
        while (
            ...
            || player.HandsController?.IsInInteractionStrictCheck() == true
            || player.HandsController is Player.FirearmController firearm && firearm.IsInReloadOperation()
        )
        {
            if (Time.realtimeSinceStartup >= until)
                throw new TimeoutException("The hands operation did not settle before checkpoint capture.");
    ...
    internal void Clear(Player player)
    {
        ...
        foreach (var slot in _equipment.Keys)
            Replace(player, slot, null);
        var quest = player.Profile.Inventory.QuestRaidItems;
        if (quest != null)
            foreach (var item in quest.Grid.Items.AsValueEnumerable().ToArray())
            {
                ...
                player.InventoryController.RaiseRemoveEvent(
                    new RemoveItemEventArgs(item, address, CommandStatus.Begin, player.InventoryController)
                );
                player.InventoryController.RaiseRemoveEvent(
                    new RemoveItemEventArgs(item, address, CommandStatus.Succeed, player.InventoryController)
                );
            }
    ...
    internal void Restore(Player player)
    {
        ...
        foreach (var pair in _equipment)
            Replace(player, pair.Key, pair.Value?.Deserialize());
        var quest = player.Profile.Inventory.QuestRaidItems;
        if (_questItems.Count > 0 && quest == null)
            throw new InvalidOperationException("The native quest inventory is unavailable.");
        foreach (var saved in _questItems)
        {
            var item = saved.Item.Deserialize();
            var result = quest!.Grid.AddItemWithoutRestrictions(
                item,
                new LocationInGrid(saved.Location.x, saved.Location.y, saved.Location.r)
            );
            if (result.Failed)
                throw new InvalidOperationException(result.Error.ToString());
            player.InventoryController.RaiseAddEvent(
                new AddItemEventArgs(item, item.Parent, CommandStatus.Begin, player.InventoryController)
            );
            player.InventoryController.RaiseAddEvent(
                new AddItemEventArgs(item, item.Parent, CommandStatus.Succeed, player.InventoryController)
            );
        }
        var bindings = player.InventoryController.FastAccess.BoundItems;
        bindings.Clear();
        foreach (var pair in _bindings)
            if (player.Equipment.TryFindItem(pair.Value, out var item))
                bindings.Add(pair.Key, item);
        var limits = player.Profile.Inventory.DiscardLimits;
        limits.Clear();
        foreach (var pair in _discardLimits)
            limits.Add(pair.Key, pair.Value);
        player.RecalculateEquipmentParams();
        foreach (var pair in _equipment)
            RequireItems(
                pair.Value,
                ItemBinarySerializer.SerializeItem(player.Equipment.GetSlot(pair.Key).ContainedItem, player.SearchController)
            );
        foreach (var saved in _questItems)
        {
    ...
    private static void Replace(Player player, EquipmentSlot key, Item? item)
    {
        ...
        if (slot.ContainedItem != null)
        {
            ...
            if (result.Failed)
                throw new InvalidOperationException(result.Error.ToString());
            player.InventoryController.RaiseRemoveEvent(
                new RemoveItemEventArgs(old, address, CommandStatus.Begin, player.InventoryController)
            );
            player.InventoryController.RaiseRemoveEvent(
                new RemoveItemEventArgs(old, address, CommandStatus.Succeed, player.InventoryController)
            );
        }
        if (item == null)
            return;
