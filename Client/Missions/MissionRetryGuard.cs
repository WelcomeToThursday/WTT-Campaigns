...
internal sealed class MissionRetryGuard : IDisposable
{
    ...
    {
        if (_current != null) throw new InvalidOperationException("Another mission already owns defeat handling.");
        Install();
        _player = player; _defeated = defeated;
        _actors.Add(player);
        _current = this;
    }
    ...
    internal void Hold() { if (Frozen) MissionStartupGuard.Hold(_player); }
    ...
    internal static bool TryDefer(ActiveHealthController health, EDamageType damageType) => _current != null
        && ReferenceEquals(health.Player, _current._player) && !Kill(health, damageType);
    [HarmonyPriority(Priority.First)]
    private static bool Kill(ActiveHealthController __instance, EDamageType damageType)
    {
        var guard = _current;
        if (guard == null || !ReferenceEquals(__instance.Player, guard._player)) return !Blocks(__instance.Player);
    ...
    {
        if (!Blocks(__instance.Player)) return true;
        __result = 0; return false;
    }
    private static bool BotUpdate(BotOwner __instance) => __instance.BotState != EBotState.Active
        || __instance.Tactic?.SubTactic == null || !Blocks(__instance.GetPlayer);
    private static bool QuestUpdate(ConditionalController<Quest> __instance) => _current is not { Frozen: true } guard
        || !ReferenceEquals(__instance.Profile, guard._player.Profile);
    private static void Changed(ActiveHealthController __instance)
    {
        var guard = _current;
        if (guard == null || !ReferenceEquals(__instance.Player, guard._player) || guard._defeatRaised) return;
    ...
    private static void Install()
    {
        ...
        void Prefix(Type type, string native, string hook) => harmony.Patch(
