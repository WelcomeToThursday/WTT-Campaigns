...
{
    private sealed class Choice { internal WildSpawnType Role; }
    ...
    internal EncounterBrainChoice(string id, WildSpawnType? restore)
    {
        if (!_installed)
        {
            ...
            foreach (var name in new[] { "GetRandomisedPlayerScavType", "GetAssaultScavWildSpawnType", "GetPmcWildSpawnType" })
                harmony.Patch(AccessTools.Method(typeof(AIBrainSpawnWeightAdjustment), name)
        ...
        _id = id; _restore = restore;
        Active.Add(id, this);
    }
    private static bool Prefix(BotOwner __0, ref WildSpawnType __result)
    {
        if (!Active.TryGetValue(__0.ProfileId, out var scope) || !scope._restore.HasValue) return true;
        __result = scope._restore.Value;
        return false;
    }
    private static void Postfix(BotOwner __0, WildSpawnType __result)
    {
        if (!Active.ContainsKey(__0.ProfileId)) return;
        Choices.GetValue(__0, _ => new Choice()).Role = __result;
    }
    internal static WildSpawnType Capture(BotOwner bot) => Choices.TryGetValue(bot, out var choice)
        ? choice.Role : bot.Profile.Info.Settings.Role;
    public void Dispose()
    {
        if (Active.TryGetValue(_id, out var current) && ReferenceEquals(current, this)) Active.Remove(_id);
    }
}
