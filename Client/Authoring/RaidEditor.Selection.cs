...
public sealed partial class RaidEditor
{
    ...
    private bool CanTransformScene(string tool) => CanSceneEdit && _sceneTab != "Catalog" && ScenePoint != null
        && SceneSelectionTarget && _sceneSelectionError.Length == 0 && ScenePoint is not MapObjectEdit { Operation: "Hide" }
        && (tool != "Scale" || ScenePoint is MapObjectEdit { Target.Kind: "Prop" }
    ...
    private void SetSceneSelectionPose(Transform target, MapTarget? binding, string error)
    {
        ...
        {
            Id = MapId(), Name = target.name, Location = _session!.Location, Scene = target.gameObject.scene.name,
            Position = ZoneRuntime.Vector(target.position), Rotation = ZoneRuntime.Vector(target.eulerAngles),
            Scale = ZoneRuntime.Vector(target.lossyScale), Target = binding ?? new MapTarget(), Operation = "Move",
        };
    ...
    {
        if (!CanTransformScene("Move") || _sceneSelectionPose == null || MapPoint != null) return;
    ...
    {
        if (SceneWorkspace && !CanTransformScene(field == "Size" ? "Scale" : "Move")) return;
        if (!SceneWorkspace || !_centerAnchor || field == "Position" || !CanTransformScene(field == "Size" ? "Scale" : "Rotate")
            || !TrySelectionBounds(out var bounds) || !SceneSelectionTarget)
        {
            EditPoint(action);
            return;
        }
        ...
        try
        {
            EditPoint(point =>
            {
                ...
                action(point);
                if (Newtonsoft.Json.Linq.JToken.DeepEquals(Newtonsoft.Json.Linq.JObject.FromObject(before), Newtonsoft.Json.Linq.JObject.FromObject(point))) return;
                _mapScene!.Reconcile(Layout, MapPoint == null ? point as MapObjectEdit : null);
                if (_mapScene.TargetErrors.Count > 0) throw new InvalidOperationException(_mapScene.TargetErrors[0]);
    ...
    {
        foreach (var renderer in _sceneRenderers) yield return renderer;
        if (_mapScene != null)
            foreach (var renderer in _mapScene.SpawnRenderers()) yield return renderer;
    }
    ...
    {
        if (!_camera) return;
        var target = ScenePicking.Pick(_camera!.ScreenPointToRay(Input.mousePosition), PickableRenderers(), t => _mapScene?.RecordAt(t) != null);
        if (target)
        {
            EnterSceneSelection();
    ...
    {
        if (!SceneWorkspace || _sceneTab == "Catalog") return;
        ...
        {
            var v = group == "Position" ? point.Position : group == "Rotation" ? point.Rotation : (point as MapObjectEdit)?.Scale;
            for (var i = 0; i < 3; i++)
            {
                var name = "Map" + group + "XYZ"[i];
                view.Value(name, (v == null ? 0 : i == 0 ? v.X : i == 1 ? v.Y : v.Z).ToString("0.###", System.Globalization.CultureInfo.InvariantCulture));
        ...
        var target = SceneSelectionTarget;
        view.Text("MapDetails", target ? (ScenePath(target!) ?? target!.name) + "\n"
            + target!.GetComponents<Component>().AsValueEnumerable().Where(c => c).Select(c => c.GetType().Name).JoinToString(", ") : point.Scene);
        ...
        if (editable && target && point is MapObjectEdit { Target.Kind: "Prop", Operation: "Move" }
        ...
        if (editable && !CanTransformScene("Scale"))
            view.Text("SceneInfo", "Move and rotate are available. " + (point is MapObjectEdit { Target.Kind: "Prop" } && target
                ? MapSceneAdapter.ScaleRestriction(target!) : "Native loot and containers retain their original size."));
        view.Windows.SetTooltip("Scale", editable && point is MapObjectEdit { Target.Kind: "Prop" } && target
            && MapSceneAdapter.ScaleRestriction(target!) is { Length: > 0 } scaleReason
            ? scaleReason : "Resize a selected static prop or volume. Native loot and containers retain their original size.");
    }
}
