...
internal sealed class ScenePreviewCache<T> where T : class
{
    ...
    internal ScenePreviewCache(int limit, Action<T> release) { _limit = limit; _release = release; }
    ...
    internal void Retry(string key) { _errors.Remove(key); _requested.Remove(key); }
    internal bool Complete(int generation, string key, T value, string protectedKey)
    {
        if (generation != Generation) { _release(value); return false; }
        _requested.Remove(key);
        if (_ready.TryGetValue(key, out var old)) _release(old);
        else if (_ready.Count >= _limit)
        {
            string? victim = null;
            foreach (var candidate in _ready.Keys)
                if (candidate != protectedKey) { victim = candidate; break; }
            if (victim == null) { _release(value); return false; }
    ...
    internal void Fail(int generation, string key, string error)
    {
        ...
        {
            string? first = null;
            foreach (var entry in _errors.Keys) { first = entry; break; }
            if (first != null) _errors.Remove(first);
        }
        _errors[key] = error;
    }
    internal void Clear()
    {
        Generation++;
        foreach (var value in _ready.Values) _release(value);
        _ready.Clear();
        _errors.Clear();
        _requested.Clear();
    }
