...
internal static class NativeContainerLibrary
{
    ...
    private static Manifest Catalog()
    {
        ...
        if (manifest?.Schema != 1)
            throw new InvalidOperationException("The native container library needs rebuilding.");
    ...
    internal static IEnumerable<SceneCatalogEntry> Entries()
    {
        foreach (var entry in Catalog().Entries)
            ...
            {
                Id = SceneAssetRules.Identity(BundleKey, entry.Asset),
                Name = entry.Name,
                AssetTarget = new MapTarget
                {
                    ...
                    Fingerprint = SceneAssetRules.Identity(BundleKey, entry.Asset),
                },
            };
    }
    ...
    private static async Task<AssetBundle> LoadBundle()
    {
        ...
        return _bundle = request.assetBundle
            ? request.assetBundle
            : throw new InvalidOperationException("Native container library failed to load.");
    }
