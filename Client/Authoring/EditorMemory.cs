...
internal static class EditorMemory
{
    ...
    internal static void Enable()
    {
        ...
        harmony.Patch(
            AccessTools.PropertySetter(typeof(GarbageCollector), nameof(GarbageCollector.GCMode)),
        ...
        harmony.Patch(
            AccessTools.PropertySetter(typeof(InGameMemoryManagement), nameof(InGameMemoryManagement.GCEnabled)),
    ...
    {
        if (!_applying)
            Policy.Filter(value ? GarbageCollector.Mode.Enabled : GarbageCollector.Mode.Disabled);
    }
    ...
    internal static void Tick(bool active)
    {
        ...
        try
        {
            ...
            if (EditorDiagnostics.Enabled)
                Plugin.LogInfo(
                    active
                        ? "Editor memory: automatic garbage collection enabled for this editor map."
