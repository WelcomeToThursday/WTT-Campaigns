...
internal sealed class CampaignTestMode : MonoBehaviour
{
    ...
    private static string _editorSession = "", _draft = "", _layout = "";
    private static bool _busy, _heartbeat, _ended, _returnFlushed, _resetFlushed;
    private static string? _resetTestId;
    private static string _status = "";
    private float _nextHeartbeat;
    private static TextMeshProUGUI? _label;
    private static DefaultUIButton? _reset, _return;
    ...
    internal static async Task Enter(string editorSession, string draft, string layout)
    {
        ...
        catch
        {
            ...
            if (_session != null)
            {
                ...
                catch (Exception recoveryError) { Plugin.Error(recoveryError); }
            }
            throw;
        }
        finally { _busy = false; }
    ...
    private static async Task LoadCharacter()
    {
        ...
        if (_snapshot.EffectiveProfileId != _session.ProfileId || _snapshot.SeasonId != _session.SeasonId
    ...
    private static async Task<CampaignTestResponse> Call(string action, string? testId = null)
    {
        var response = JsonConvert.DeserializeObject<CampaignTestResponse>(
            await RequestHandler.PostJsonAsync("/wtt-campaigns/test-campaign/" + action,
                JsonConvert.SerializeObject(new CampaignTestRequest
                {
    ...
    {
        if (!Active || _busy || Plugin.InRaid) return;
        _busy = true;
        ...
        catch (Exception e) { _status = e.Message; Plugin.Error(e); }
        finally { _busy = false; }
    }

    private static async void Return()
    {
        if (!Restricted || _busy || Plugin.InRaid) return;
        _busy = true;
    ...
    {
        if (_label) { _label!.text = _status; _label.gameObject.SetActive(Restricted && !Plugin.InRaid); }
        if (_reset) _reset!.Interactable = Active && !_busy && !Plugin.InRaid;
        if (_return) _return!.Interactable = !_busy && !Plugin.InRaid;
        if (!Active || _busy || _heartbeat || Time.realtimeSinceStartup < _nextHeartbeat) return;
        ...
        catch (Exception e) { _status = "Campaign test: " + e.Message; Plugin.Error(e); }
        finally { _heartbeat = false; }
    }
    ...
    internal static void AttachMenu(MenuScreen menu)
    {
        ...
        if (existing) _label = existing.GetComponent<TextMeshProUGUI>();
        else
        {
