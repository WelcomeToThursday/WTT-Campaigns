...
{
    internal enum Area { FrameUpdate, LateUpdate, Hud, Environment, TerrainScan, Presentation, Geometry, Index, OtherEditor, NativeCulling, NativeEffects, Count }
    private static readonly EditorTiming[] Timings = new EditorTiming[(int)Area.Count];
    private static readonly string[] CounterNames = { "Object Count", "Texture Count", "Mesh Count", "Material Count", "Render Texture Count" };
    private static readonly ProfilerRecorder[] Counters = new ProfilerRecorder[CounterNames.Length];
    private static bool _inRaid, _capturing, _failed, _allocationCounterAvailable;
    private static float _started, _lastSample, _lastFrame, _worstFrame;
    private static long _mainAllocated;
    private static int _collections, _frames, _stutters;
    ...
    internal readonly struct Scope : IDisposable
    {
        ...
        private readonly long _time, _allocated;
        ...
        public void Dispose()
        {
            if (_measure && _capturing)
                Timings[(int)_area].Add(Stopwatch.GetTimestamp() - _time,
    ...
    internal static void Tick(bool inRaid)
    {
        try
        {
            ...
            if (frame >= 50) _stutters++;
            _frames++;
            if (now - _lastSample < 5) return;
            Report(now);
            if (now - _started >= 180)
            {
                Stop();
    ...
    private static void Start()
    {
        ...
        for (var i = 0; i < Counters.Length; i++)
        {
            ...
            try { Counters[i] = ProfilerRecorder.StartNew(ProfilerCategory.Memory, CounterNames[i], 1); }
            catch { Counters[i] = default; }
        }
        ...
        _worstFrame = 0;
        _capturing = true;
        Plugin.LogInfo("Editor memory capture started: five-second samples for three minutes; timings include nested scopes. Build "
    ...
    private static void Report(float now)
    {
        ...
        text.AppendFormat(System.Globalization.CultureInfo.InvariantCulture,
            ...
            now - _started, Memory(_process.PrivateMemorySize64), Memory(_process.WorkingSet64),
            Profiler.GetMonoUsedSizeLong() / mb, Profiler.GetMonoHeapSizeLong() / mb,
            Profiler.GetTotalAllocatedMemoryLong() / mb, Profiler.GetTotalReservedMemoryLong() / mb,
            Memory(Profiler.GetAllocatedMemoryForGraphicsDriver()), AllocationRate((allocated - _mainAllocated) / mb / elapsed),
            GarbageCollector.GCMode, collections - _collections, _frames / elapsed, _worstFrame, _stutters);
        for (var i = 0; i < Counters.Length; i++)
            text.Append(" | ").Append(CounterNames[i]).Append('=').Append(Counters[i].Valid && Counters[i].Count > 0 ? Counters[i].LastValue.ToString() : "unavailable");
        for (var i = 0; i < Timings.Length; i++)
        {
            var timing = Timings[i];
            if (timing.Calls == 0) continue;
            text.AppendFormat(System.Globalization.CultureInfo.InvariantCulture, " | {0}: calls={1} alloc={2} total={3:F1}ms max={4:F1}ms",
                (Area)i, timing.Calls, AllocationRate(timing.Allocated / mb / elapsed),
                timing.Ticks * 1000d / Stopwatch.Frequency, timing.MaxTicks * 1000d / Stopwatch.Frequency);
        }
    ...
    private static string Memory(long bytes) => bytes > 0
        ? (bytes / (1024d * 1024)).ToString("F1", System.Globalization.CultureInfo.InvariantCulture) + "MiB" : "unavailable";

    private static string AllocationRate(double rate) => _allocationCounterAvailable
        ? rate.ToString("F2", System.Globalization.CultureInfo.InvariantCulture) + "MiB/s" : "unavailable";
