...
{
    private float CameraSpeed => float.IsNaN(_cameraSpeed.Value) || float.IsInfinity(_cameraSpeed.Value)
        ? 6f : Mathf.Clamp(_cameraSpeed.Value, .25f, 96f);
    ...
    private void BindCameraControls(RaidEditorView view)
    {
        void Set(float value)
        {
            ...
            view.Get<UnityEngine.UI.InputField>("CameraSpeed").SetTextWithoutNotify(CameraSpeed.ToString("0.##", CultureInfo.InvariantCulture));
            Refresh(false);
        }
        view.Button("CameraSlower", () => Set(CameraSpeed / 2));
        view.Button("CameraFaster", () => Set(CameraSpeed * 2));
        view.Input("CameraSpeed", value =>
        {
            if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out var speed)
                && !float.IsNaN(speed) && !float.IsInfinity(speed) && speed >= .25f && speed <= 96f)
                Set(speed);
            else
            {
