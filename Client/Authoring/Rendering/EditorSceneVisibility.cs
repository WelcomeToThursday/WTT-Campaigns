...
internal sealed class EditorSceneVisibility : IDisposable
{
    ...
    {
        if (!owner || owner.bakeGroups == null)
            return;
        if (_owners.TryGetValue(owner, out var previous))
        {
            if (!initialized && previous == owner.bakeGroups)
                return;
        ...
        foreach (var group in owner.bakeGroups)
            if (group?.RuntimeGroupData != null && _groups.Add(group))
                group.Toggle(true);
    }
    ...
    private static void BeforeToggle(PerfectCullingBakeGroup __instance, ref bool rendererEnabled)
    {
        ...
        if (_active?._groups.Contains(__instance) == true)
    ...
    internal static void Enable()
    {
        ...
        harmony.Patch(
            AccessTools.Method(typeof(PerfectCullingBakeGroup), nameof(PerfectCullingBakeGroup.Toggle)),
            prefix: new HarmonyMethod(typeof(EditorSceneVisibility), nameof(BeforeToggle))
        );
        harmony.Patch(
            AccessTools.Method(typeof(CullingGridVisibilityQueryResult), nameof(CullingGridVisibilityQueryResult.TestAABB)),
            prefix: new HarmonyMethod(typeof(EditorSceneVisibility), nameof(BeforeTest))
        );
        harmony.Patch(
            AccessTools.Method(typeof(PerfectCullingCrossSceneGroup), nameof(PerfectCullingCrossSceneGroup.InitializeRuntime)),
    ...
    public void Dispose()
    {
        ...
        {
            if (!entry.Key || entry.Key.bakeGroups != entry.Value)
                continue;
            foreach (var group in entry.Value)
                if (group != null && _groups.Contains(group))
                    try
                    {
                        group.Toggle(group.IsEnabled);
                    }
