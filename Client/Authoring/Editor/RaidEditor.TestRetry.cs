...
public sealed partial class RaidEditor
{
    ...
    {
        _testRetryGuard!.Release();
        _editorDirector?.Resume();
        _aiRuntime?.Resume();
    }
    ...
    private void CheckTestFailure()
    {
        ...
        if (failure.Length == 0)
            return;
        if (_testRetryGuard == null)
        {
            _aiPreviewStatus = failure;
            EndAiPreview();
            return;
        }
    ...
    private bool HoldTestFailure()
    {
        ...
        if (!_editorMissionProgressPending && !_editorMissionRetrying && !_testFailureShown && _testFailure.Length > 0)
        {
            ...
            var name = _testCheckpoint?.Id is not { Length: > 0 }
                ? "Mission start"
                : _editorMissionLayout!.Checkpoints.Find(c => c.Id == _testCheckpoint.Id)?.Name ?? "Checkpoint";
            _testHud?.ShowFailure(
                _testFailure,
                name,
                _testRestoreBroken || _testCheckpoint == null
                    ? null
                    : () =>
                    {
                        _ = RestoreTestCheckpoint();
                    },
                () => EndAiPreview()
            );
        }
    ...
    {
        if (_editorMissionRetrying || _editorMissionProgressPending || _testRestoreBroken || _testCheckpoint == null)
            return;
        ...
        try
        {
            ...
            if (prepared.Run!.CheckpointId != _testCheckpoint.Id)
                throw new InvalidDataException("The test checkpoint identity changed.");
            _editorMissionTest = prepared;
            _editorDirector!.Dispose();
            _editorDirector = null;
            _aiRuntime!.Reset(preserveWorld: true);
            _aiRuntime = null;
            ...
            await _aiRuntime.BeginAsync(
                new EncounterRuntimeContext
                {
                    ...
                    RaidId = _session!.RaidId,
                    LayoutId = _editorMissionLayout!.Id,
                    LayoutRevision = _session.ContentVersion,
            ...
            if (_testCheckpoint.Id.Length == 0)
    ...
    {
        if (_aiRuntime != null)
            _aiRuntime.RetryGuard = null;
        _testRetryGuard?.Dispose();
