...
internal sealed class EditorNumericDrag
{
    ...
    private void Down(PointerDownEvent evt)
    {
        ...
        if (!double.TryParse(_field.value, NumberStyles.Float, CultureInfo.InvariantCulture, out _value) || double.IsNaN(_value) || double.IsInfinity(_value))
            return;
    ...
    private void Move(PointerMoveEvent evt)
    {
        ...
        if (!_moved && Mathf.Abs(evt.position.x - _startX) < 3)
            return;
        _moved = true;
        var sensitivity = evt.shiftKey ? .1 : evt.ctrlKey ? 10 : 1;
        _value = Math.Max(_min, Math.Min(_max, _value + (evt.position.x - _lastX) * _step * sensitivity));
        _lastX = evt.position.x;
        _field.SetValueWithoutNotify((_integer ? Math.Round(_value) : _value).ToString(_integer ? "0" : "0.###", CultureInfo.InvariantCulture));
        evt.StopPropagation();
        evt.PreventDefault();
    }
