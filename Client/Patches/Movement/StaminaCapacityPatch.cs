...
internal class StaminaCapacityPatch(string methodName) : ModulePatch("WTT.Campaigns." + methodName)
{
    ...
    private static void Postfix(Physical __instance, MethodBase __originalMethod, ref float __result)
    {
        ...
        var bodyPart = __originalMethod.Name == nameof(Physical.GetHandsCapacityFunc) ? "arms" : "legs";
        __result = Math.Max(1, __result + Plugin.Effects.Offset("stamina_scale_body_parts", bodyPart));
    }
}
