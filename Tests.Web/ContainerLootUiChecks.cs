...
internal static class ContainerLootUiChecks
{
    ...
    {
        check(
            typeof(WTT.Campaigns.Server.Web.Pages.MissionEditor)
                .GetCustomAttributes<RouteAttribute>()
                .Any(r => r.Template == "/wtt-campaigns/creator/missions"),
            "Mission editor has its own browser route"
        );
        check(
            typeof(WTT.Campaigns.Server.Web.Pages.MissionEditor)
                .GetCustomAttributes<Microsoft.AspNetCore.Authorization.AuthorizeAttribute>()
                .Any(a => a.Policy == "Administrator"),
            "Standalone mission editing retains administrator authorization"
        );
        var container = new MapObjectEdit
        {
            Id = "crate",
            Name = "Crate",
            Operation = "Copy",
            Target = new() { Kind = "AssetContainer", Template = item },
            Container = new() { Mode = "Fixed" },
        };
        var untouched = new MapObjectEdit
        {
            Id = "other",
            Operation = "Copy",
            Target = new() { Kind = "Container", Template = item },
        };
        ...
        await renderer.Dispatcher.InvokeAsync(async () =>
        {
            ...
            async Task Call(string name, params object[] args) =>
                await (Task)
                    typeof(MissionContainerLoot).GetMethod(name, BindingFlags.NonPublic | BindingFlags.Instance)!.Invoke(editor, args)!;
            check(
                container.Container.Contents.Count == 0 && untouched.Container == null,
                "Opening container loot does not create or change settings"
            );
            await Call("AddItem", item);
            await Call("AddItem", other);
            check(
                container.Container.Contents.Count == 2 && host.Changes == 2,
                "Fixed loot additions notify the draft and preserve separate entries"
            );
            ...
            check(
                container.Container.Contents[0].Count == 8 && host.Changes == 3,
                "Invalid quantity is rejected without marking the draft changed"
            );
            ...
            check(
                container.Container.Contents.Count == 1 && container.Container.Contents[0].Template == other,
                "Removing a row changes only that fixed entry"
            );
            ...
            check(
                renderer
                    .Components<ItemPreviewImage>()
                    .Any(p => p.Component.Campaign == season.Id && p.Component.Items.Any(i => i.Template == other)),
                "Container rows use the assort image component with campaign and item identity"
            );
            var saved = SeasonCompiler.Copy(season);
            check(
                saved.MapLayouts[0].Objects[0].Container!.Contents[0].Template == other,
                "Container loot survives the campaign serialization roundtrip"
            );
        });
    }
