...
internal static class EditorMissionCreationChecks
{
    ...
    internal static void Run(Action<bool, string> check)
    {
        ...
        try
        {
            ...
            var store = (SeasonRepository)
                Activator.CreateInstance(typeof(SeasonRepository), BindingFlags.Instance | BindingFlags.NonPublic, null, [folder], null)!;
            var existing = store.Save(
                new DraftEnvelope
                {
            ...
            var request = new EditorSessionRequest
            {
            ...
            void Reject(string owner, EditorSessionRequest input, string reason)
            {
                ...
                try
                {
                    editor.CreateMission(owner, input);
                }
            ...
            session.Ready = true;
            foreach (
                var (name, location) in new[] { (" ", "woods"), (new string('x', 121), "woods"), ("Mission", ""), ("Mission", "hideout") }
            )
                Reject(
                    session.Owner,
                    new()
                    ...
                    "Invalid mission input leaves no draft"
                );
            ...
            check(
                mission.Name == "Factory rescue" && layout.Name == mission.Name && created.Definition.Name == mission.Name,
                "Creation persists the trimmed mission name consistently"
            );
            check(
                layout.Location == request.Location && mission.LayoutId == layout.Id,
                "Creation persists the chosen map and mission ownership"
            );
            check(
                created.Definition.MissionPackage != null
                    && response.Drafts.Any(d => d.Id == created.Id)
                    && response.Layouts.Single().Location == request.Location,
                "New mission immediately appears in the home response"
            );
            ...
            check(
                blank.Definition.Name == "New mission" && blank.Definition.MapLayouts.Single().Location == "",
