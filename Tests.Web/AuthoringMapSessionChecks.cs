...
internal static class AuthoringMapSessionChecks
{
    internal static void Run(Action<bool, string> check)
    {
        ...
        foreach (var version in new[] { -1, 0, 1, 8, int.MaxValue })
        {
            request.Version = version;
            check(!Accepts(), $"Editor socket rejects unsupported map protocol {version}");
        }
        request.Version = 7;
        check(!Accepts("other-owner"), "Version 7 retains owner isolation");
        request.EditorSessionId = Guid.NewGuid().ToString("N");
        check(!Accepts(), "Version 7 rejects stale session tokens");
        request.EditorSessionId = session.Id;
        request.Location = "shoreline";
        check(!Accepts(), "Version 7 rejects the wrong map");
        request.Location = session.Location;
        session.Ready = false;
        check(!Accepts(), "Version 7 rejects retired sessions");
        session.Ready = true;
        session.Contact = now - TimeSpan.FromMinutes(2);
        check(!Accepts(), "Version 7 rejects expired sessions");
    }
}
