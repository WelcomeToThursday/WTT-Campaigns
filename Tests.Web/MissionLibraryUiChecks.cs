...
internal static class MissionLibraryUiChecks
{
    internal static async Task Run(IServiceProvider services, Action<bool, string> check)
    {
        ...
        try
        {
            ...
            var rehearsal = (EditorTestMissionResponse)
                prepare.Invoke(
                    service,
                    [
                        session,
                        new EditorTestMissionRequest
                        {
                            ...
                            MissionId = draft.Definition.Missions.Single().Id,
                        },
                    ]
                )!;
            check(
                rehearsal.Descriptor?.Definition.QuestId == "" && rehearsal.RunId.Length > 0,
                "Disposable mission preparation accepts an independent mission without a campaign quest"
            );
            check(
                JsonConvert.SerializeObject(repository.Load(draft.Id)) == before,
                "Disposable preparation preserves its source mission draft"
            );
            var pageType = typeof(WTT.Campaigns.Server.Web.Pages.Creator);
            var page = new WTT.Campaigns.Server.Web.Pages.Creator
            {
                MissionEditor = true,
                DraftQuery = draft.Id,
                SectionQuery = "Layouts",
            };
            ...
            check(
                ((DraftEnvelope?)currentDraft.GetValue(page))?.Id == draft.Id && (string?)currentTab.GetValue(page) == "Layouts",
                "Direct layout navigation loads the requested saved draft and selects Layouts"
            );
            currentDraft.SetValue(page, null);
            applyQuery.Invoke(page, null);
            check(
                ((DraftEnvelope?)currentDraft.GetValue(page))?.Id == draft.Id,
            ...
            applyQuery.Invoke(page, null);
            check(
                ReferenceEquals(openedDraft, currentDraft.GetValue(page)) && (string?)currentTab.GetValue(page) == "Missions",
                "Switching mission workspace query preserves the open draft"
            );
            currentDraft.SetValue(page, null);
            var levelPage = new WTT.Campaigns.Server.Web.Pages.Creator
            {
                LevelEditor = true,
                DraftQuery = draft.Id,
                LayoutQuery = layout.Id,
            };
            ...
            var levelDraft = repository.Save(
                new DraftEnvelope
                {
                    Id = SeasonRepository.NewId(),
            ...
            applyQuery.Invoke(levelPage, null);
            check(
                ((DraftEnvelope?)currentDraft.GetValue(levelPage))?.Id == levelDraft.Id,
            ...
            foreach (var search in new[] { "", "Interchange" })
            {
                ...
                check(
                    ActualLibrarySearch(pageTree) == search,
            ...
            await renderer.Dispatcher.InvokeAsync(async () =>
            {
                ...
                check(
                    text.Contains("Mission layout") && !text.Contains("Story quest") && !text.Contains("Completion objective"),
                    "Independent mission editing offers layout and briefing without requiring quest fields"
                );
                check(
                    renderer.Components<MissionLogicFields>().Count() == 1,
                    "Independent mission editor retains objective and event editing"
                );
                var campaign = new DraftEnvelope
                {
                    Id = "legacy-draft",
                    Definition = new SeasonDefinition
                    {
                        Name = "New campaign",
                        Missions = [new() { Name = "Test", LayoutId = "mission-layout" }],
                        MapLayouts =
                        [
                            new()
                            {
                ...
                check(
                    legacyText.Contains("New campaign")
                        && legacyText.Contains("Test layout")
                        && legacyText.Contains("Edit missions and layouts"),
                ...
                check(
                    mapText.Contains("Test layout") && mapText.Contains("Interchange") && mapText.Contains("Open layouts"),
                ...
                check(
                    renderer.Text(levelComponent.Id).Contains("Ordinary PMC level")
                        && !renderer.Text(levelComponent.Id).Contains("Test layout"),
                ...
                check(
                    renderer.Text(missionComponent.Id).Contains("Test layout")
                        && !renderer.Text(missionComponent.Id).Contains("Ordinary PMC level")
                        && !renderer.Text(missionComponent.Id).Contains("Apply in normal raids"),
                    "Mission layouts cannot be enabled as normal levels in the web editor"
                );
            });
    ...
    private static string? ActualLibrarySearch(RenderTreeBuilder tree)
    {
        ...
        for (var i = 0; i < frames.Count; i++)
        {
            ...
            if (
                ...
                && frame.ComponentType == typeof(CampaignDraftMissions)
            )
            {
                for (var j = i + 1; j < i + frame.ComponentSubtreeLength; j++)
                    if (
                        frames.Array[j].FrameType == Microsoft.AspNetCore.Components.RenderTree.RenderTreeFrameType.Attribute
                        && frames.Array[j].AttributeName == "Search"
                    )
                        return (string?)frames.Array[j].AttributeValue;
            }
            if (
                frame.FrameType == Microsoft.AspNetCore.Components.RenderTree.RenderTreeFrameType.Attribute
                && frame.AttributeValue is RenderFragment fragment
            )
            {
