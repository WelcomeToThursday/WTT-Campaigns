...
internal static class MissionLogicUiChecks
{
    public static async Task Run(IServiceProvider services, Action<bool, string> check)
    {
        ...
        {
            Encounters = [new() { Id = "enc", Name = "Encounter", Waves = [new() { Id = "wave", Roster = [
                new() { Id = "single", Count = 1 }, new() { Id = "single2", Count = 1 }, new() { Id = "group", Count = 2 }
            ] }] }],
        };
        var objective = new MissionObjective { Id = "goal", TargetKind = "Roster", TargetIds = ["single", "group"] };
        ...
        await renderer.Dispatcher.InvokeAsync(async () =>
        {
            ...
            check(Choice("Target group").Choices.Select(c => c.Id).SequenceEqual(new[] { "Roster" }), "Individual goals offer only roster targets");
            check(Choice("Actor").Choices.Select(c => c.Id).SequenceEqual(new[] { "single", "single2" }), "Actor picker excludes multiple-bot rosters");
            ...
            check(objective.TargetKind == "Encounter" && objective.TargetIds.Count == 0, "Survive changes target kind and clears incompatible actor IDs");
            ...
            check(Choice("Target group").Choices.Count() == 3 && !renderer.Components<MissionChoice>().Any(c => c.Component.Label == "Actor"), "Group goals retain all group kinds and multiple selection");
            ...
            check(mission.Objectives.Count == 1 && layout.Encounters[0].Waves[0].Roster[2].Count == 2, "Choice filtering preserves authored objectives and roster counts");
        });
    }
