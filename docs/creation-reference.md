...
The duplicate faction preview fix removes inherited `MenuPlayerPoser` children from the inactive screen clone before initializing previews. Unity copies these loaded children, but the native `PlayerModelView` loader starts fresh and cannot release them when it loads a new model. Copied bodies are disposed and deactivated before destruction. Source selection also excludes seasonal clones that may still be awaiting async cleanup. In-game regression checks: open creation after native profile creation, select both factions, move to appearance and back, then cancel and reopen; each faction should show one model, with head selection and camera transitions still working. This fix requires an in-game visual retest.
