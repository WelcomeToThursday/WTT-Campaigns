...
The loading overlay displays LOADING CHARACTER, the selected character's name, and PVE ZONE / REGULAR or PVE SEASON with that character's season name beneath the native logo. The text uses the profile screen's Bender font, muted pale text and cyan accents. All three lines shimmer together using a brightness curve sampled from the native logo's sprite animation and driven by its current playback phase. Text opacity stays between 65% and 100% for readability; hiding the loader stops the animation, and each selection restarts it with the logo. Labels refresh on every selection, allow longer names to fit, and do not interpret character names as rich text.
